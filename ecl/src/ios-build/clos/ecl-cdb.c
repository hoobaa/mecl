   ...
   v14s = cl_open(5, v2stream, ECL_SYM("DIRECTION",1218), ECL_SYM("INPUT",1254), ECL_SYM("ELEMENT-TYPE",1224), VV[8]);
   {
    ...
    } else {
    {
     ...
     volatile cl_object _ecl_inner_frame = ecl_stack_frame_open(cl_env_copy,(cl_object)&_ecl_inner_frame_aux,0);
     {
      ...
      if (Null(v14s)) { goto L53; }
      cl_close(1, v14s);
L53:;cl_env_copy->values[0]=ecl_stack_frame_pop_values(v17);
     }
...
L45:;
          ...
          if (!(ecl_number_compare(v8i,v9)>=0)) { goto L60; }
          goto L38;
L60:;
   ...
   v8s = cl_open(5, v2stream, ECL_SYM("DIRECTION",1218), ECL_SYM("INPUT",1254), ECL_SYM("ELEMENT-TYPE",1224), VV[8]);
   {
    ...
    } else {
    {
     ...
     volatile cl_object _ecl_inner_frame = ecl_stack_frame_open(cl_env_copy,(cl_object)&_ecl_inner_frame_aux,0);
     {
      ...
      if (Null(v8s)) { goto L74; }
      cl_close(1, v8s);
L74:;cl_env_copy->values[0]=ecl_stack_frame_pop_values(v11);
     }
     ecl_stack_frame_close(_ecl_inner_frame);
    }
    }
...
#include "clos/ecl-cdb.data"
#ifdef __cplusplus
extern "C"
#endif
ECL_DLLEXPORT void _ecli2xNviZ72s5m9_W7wTOE21(cl_object flag)
{
 const cl_env_ptr cl_env_copy = ecl_process_env();
 cl_object value0;
 cl_object *VVtemp;
 ...
 #endif
 Cblock->cblock.data_text = "@EcLtAg:_ecli2xNviZ72s5m9_W7wTOE21@";
