...
#endif
        switch (method) {
        case 0:	cl_error(1, ECL_SYM("EXT::STORAGE-EXHAUSTED",1666));
                break;
        case 1: cl_cerror(2, make_constant_base_string("Extend heap size"),
                          ECL_SYM("EXT::STORAGE-EXHAUSTED",1666));
                break;
        case 2:
                return output;
        default:
...
si_gc_stats(cl_object enable)
{
	...
	if (cl_core.bytes_consed == ECL_NIL) {
        ...
        if (enable == ECL_NIL) {
                ...
                cl_core.gc_stats = 1;
                GC_print_stats = (enable == ECL_SYM(":FULL",1604));
        }
	{
#line 1240
		const cl_env_ptr the_env = ecl_process_env();
...
si_weak_pointer_value(cl_object o)
{
	...
	if (ecl_unlikely(ecl_t_of(o) != t_weak_pointer))
		FEwrong_type_only_arg(ecl_make_fixnum(/*EXT::WEAK-POINTER-VALUE*/1731), o,
                                      ecl_make_fixnum(/*EXT::WEAK-POINTER*/1729));
