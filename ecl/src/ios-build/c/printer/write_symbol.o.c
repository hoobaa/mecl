#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/printer/write_symbol.d"
			...
			if (ecl_upper_case_p(c)) {
				if ((action == ecl_case_invert) ||
				    ((action == ecl_case_upcase) &&
				     ((print_case == ECL_SYM(":DOWNCASE",1223)) ||
				      ((print_case == ECL_SYM(":CAPITALIZE",1203)) && !capitalize))))
				{
				...
				if ((action == ecl_case_invert) ||
				    ((action == ecl_case_downcase) &&
				     ((print_case == ECL_SYM(":UPCASE",1322)) ||
				      ((print_case == ECL_SYM(":CAPITALIZE",1203)) && capitalize))))
				{
					c = ecl_char_upcase(c);
				}
...
{
	cl_object print_package = ecl_symbol_value(ECL_SYM("SI::*PRINT-PACKAGE*",1029));
