...
#endif
#ifdef SIGILL
	{ SIGILL, "+SIGILL+", ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1675)},
#endif
#ifdef SIGTRAP
...
#endif
#ifdef SIGBUS
	{ SIGBUS, "+SIGBUS+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1678)},
#endif
#ifdef SIGSEGV
	{ SIGSEGV, "+SIGSEGV+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1678)},
#endif
#ifdef SIGSYS
...
handle_signal_now(cl_object signal_code, cl_object process)
{
        ...
        case t_fixnum:
                cl_cerror(4, str_ignore_signal, ECL_SYM("EXT::UNIX-SIGNAL-RECEIVED",1802),
                          ECL_SYM(":CODE",1804), signal_code);
                break;
        case t_symbol:
		/*
...
asynchronous_signal_servicing_thread()
{
	...
	for (;;) {
		...
		signal_code = ecl_gethash_safe(ecl_make_fixnum(signal_thread_msg.signo),
					       ...
					       ECL_NIL);
		if (!Null(signal_code)) {
			mp_process_run_function(4, ECL_SYM("SI::HANDLE-SIGNAL",1734),
						ECL_SYM("SI::HANDLE-SIGNAL",1734),
						signal_code,
...
# endif /* ECL_DOWN_STACK */
	...
	if (the_env->fault_address == info->si_addr) {
		...
		the_env->fault_address = info->si_addr;
		handle_or_queue(the_env, ECL_SYM("EXT::SEGMENTATION-VIOLATION",1678), sig);
	}
#else
	/*
...
_ecl_w32_exception_filter(struct _EXCEPTION_POINTERS* ep)
{
	...
	switch (ep->ExceptionRecord->ExceptionCode)
	{
		...
		case EXCEPTION_ACCESS_VIOLATION:
                        handle_signal_now(ECL_SYM("EXT::SEGMENTATION-VIOLATION",1678), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		/* Catch illegal instruction */
		case EXCEPTION_ILLEGAL_INSTRUCTION:
			handle_signal_now(ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1675), the_env->own_process);
			return EXCEPTION_CONTINUE_EXECUTION;
...
{
	int outside_ecl = ecl_import_current_thread(ECL_SYM("SI::HANDLE-SIGNAL",1734), ECL_NIL);
	mp_process_run_function(4, ECL_SYM("SI::HANDLE-SIGNAL",1734),
				ECL_SYM("SI::HANDLE-SIGNAL",1734),
				signal_code, ECL_NIL);
	...
	for (;;) {
		...
		if (status == 0) {
			...
			signal_code = ecl_gethash_safe(ecl_make_fixnum(signo),
						       ...
						       ECL_NIL);
			if (!Null(signal_code)) {
				mp_process_run_function(3, ECL_SYM("SI::HANDLE-SIGNAL",1734),
							ECL_SYM("SI::HANDLE-SIGNAL",1734),
							signal_code);
			}
		}
	...
	if (ecl_option_values[ECL_OPT_SIGNAL_HANDLING_THREAD]) {
		cl_object fun =
			ecl_make_cfun((cl_objectfn_fixed)
				      asynchronous_signal_servicing_thread,
				      ECL_SYM("SI::SIGNAL-SERVICING",1767),
				      ECL_NIL,
				      0);
		cl_object process =
