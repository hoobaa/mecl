...
mp_condition_variable_wait(cl_object cv, cl_object lock)
{
	...
	unlikely_if (ecl_t_of(cv) != t_condition_variable) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*MP::CONDITION-VARIABLE-WAIT*/1421), 1, cv,
                                     ecl_make_fixnum(/*MP::CONDITION-VARIABLE*/1398));
	}
	unlikely_if (ecl_t_of(lock) != t_lock) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*MP::CONDITION-VARIABLE-WAIT*/1421), 2, lock,
                                     ecl_make_fixnum(/*MP::LOCK*/1397));
	}
