#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/threads/process.d"
...
assert_type_process(cl_object o)
{
	if (ecl_t_of(o) != t_process)
		FEwrong_type_argument(ecl_make_fixnum(/*MP::PROCESS*/1396), o);
}
...
thread_cleanup(void *aux)
{
	...
	mp_barrier_unblock(3, process->process.exit_barrier, ECL_SYM(":DISABLE",1459), ECL_T);
...
#endif
{
	...
	*/
	ECL_CATCH_ALL_BEGIN(env) {
		...
		si_trap_fpe(ECL_SYM("LAST",455), ECL_T);
		ecl_bds_bind(env, ECL_SYM("MP::*CURRENT-PROCESS*",1399), process);

		ECL_RESTART_CASE_BEGIN(env, ECL_SYM("ABORT",77)) {
...
#line 425
	static cl_object KEYS[2] = {(cl_object)(cl_symbols+1273), (cl_object)(cl_symbols+1425)};
	cl_object name;
...
#line 425
	if (ecl_unlikely(narg < 0)) FEwrong_num_arguments(ecl_make_fixnum(1402));
#line 425
...
mp_process_preset(cl_narg narg, cl_object process, cl_object function, ...)
{
	...
	if (narg < 2)
		FEwrong_num_arguments(ecl_make_fixnum(/*MP::PROCESS-PRESET*/1408));
...
{
        cl_env_ptr env = ecl_process_env();
        ECL_CATCH_BEGIN(env,ECL_SYM("MP::SUSPEND-LOOP",1433)) {
                for ( ; ; ) {
...
{
        cl_env_ptr the_env = ecl_process_env();
        if (frs_sch(ECL_SYM("MP::SUSPEND-LOOP",1433))) {
                cl_throw(ECL_SYM("MP::SUSPEND-LOOP",1433));
        }
	ecl_return0(the_env);
}
...
{
        return mp_interrupt_process(process, ECL_SYM("MP::SUSPEND-LOOP",1433));
}

cl_object
mp_process_resume(cl_object process)
{
        return mp_interrupt_process(process, ECL_SYM("MP::BREAK-SUSPEND-LOOP",1434));
}

cl_object
mp_process_kill(cl_object process)
{
	return mp_interrupt_process(process, ECL_SYM("MP::EXIT-PROCESS",1401));
}
...
mp_process_run_function(cl_narg narg, cl_object name, cl_object function, ...)
{
	...
	ecl_va_start(args, function, narg, 2);
	if (narg < 2)
		FEwrong_num_arguments(ecl_make_fixnum(/*MP::PROCESS-RUN-FUNCTION*/1409));
	if (CONSP(name)) {
		process = cl_apply(2, ECL_SYM("MP::MAKE-PROCESS",1402), name);
	} else {
		process = mp_make_process(2, ECL_SYM(":NAME",1273), name);
	}
	cl_apply(4, ECL_SYM("MP::PROCESS-PRESET",1408), process, function,
...
mp_process_run_function_wait(cl_narg narg, ...)
{
	...
	ecl_va_list args;
	ecl_va_start(args, narg, narg, 0);
	process = cl_apply(2, ECL_SYM("MP::PROCESS-RUN-FUNCTION",1409),
                           cl_grab_rest_args(args));
        if (!Null(process)) {
...
# endif
...

	{
		...
		v->vector.fillp = 1;
		cl_core.processes = v;
		cl_core.global_lock = ecl_make_lock(ECL_SYM("MP::GLOBAL-LOCK",1442), 1);
		cl_core.error_lock = ecl_make_lock(ECL_SYM("MP::ERROR-LOCK",1443), 1);
