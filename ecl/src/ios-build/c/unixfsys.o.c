#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/unixfsys.d"
...
file_kind(char *filename, bool follow_links) {
	...
	if (dw == -1)
		...
		output = ECL_SYM(":DIRECTORY",1219);
	else
		output = ECL_SYM(":FILE",1237);
	ecl_enable_interrupts();
#else
	struct stat buf;
# ifdef HAVE_LSTAT
...
si_readlink(cl_object filename) {
	...
	if (kind == ECL_SYM(":DIRECTORY",1219)) {
...
enter_directory(cl_object base_dir, cl_object subdir, bool ignore_if_failure)
{
        ...
        if (subdir == ECL_SYM(":ABSOLUTE",1196)) {
                return cl_make_pathname(4, ECL_SYM(":DIRECTORY",1219), ecl_list1(subdir),
                                        ECL_SYM(":DEFAULTS",1216), base_dir);
        } else if (subdir == ECL_SYM(":RELATIVE",1299)) {
                /* Nothing to do */
                return base_dir;
        } else if (subdir == ECL_SYM(":UP",1321)) {
        ...
        output = cl_make_pathname(4, ECL_SYM(":DIRECTORY",1219), aux, ECL_SYM(":DEFAULTS",1216), base_dir);
...
make_absolute_pathname(cl_object orig_pathname)
{
	...
	cl_object pathname = coerce_to_file_pathname(orig_pathname);
        return ecl_merge_pathnames(pathname, base_dir, ECL_SYM(":DEFAULT",1215));
}
...
make_base_pathname(cl_object pathname)
{
        return ecl_make_pathname(pathname->pathname.host,
				 ...
				 ecl_list1(ECL_SYM(":ABSOLUTE",1196)),
				 ECL_NIL, ECL_NIL, ECL_NIL, ECL_SYM(":LOCAL",1268));
}
...
file_truename(cl_object pathname, cl_object filename, int flags)
{
	...
	if (Null(pathname)) {
...
#ifdef HAVE_LSTAT
        } else if (kind == ECL_SYM(":LINK",1266) && (flags & FOLLOW_SYMLINKS)) {
		...
		pathname = ecl_make_pathname(pathname->pathname.host,
					     ...
					     ECL_NIL, ECL_NIL, ECL_NIL, ECL_SYM(":LOCAL",1268));
                pathname = ecl_merge_pathnames(filename, pathname, ECL_SYM(":DEFAULT",1215));
        ...
        if (pathname->pathname.name == ECL_NIL &&
            pathname->pathname.type == ECL_NIL) {
                ...
                pathname->pathname.version = ECL_SYM(":NEWEST",1276);
        }
	{
#line 393
		const cl_env_ptr the_env = ecl_process_env();
...
cl_truename(cl_object orig_pathname)
{
        ...
        for (dir = pathname->pathname.directory; !Null(dir); dir = ECL_CONS_CDR(dir))
	{
        ...
        pathname = ecl_merge_pathnames(base_dir, pathname, ECL_SYM(":DEFAULT",1215));
	{
#line 419
		const cl_env_ptr the_env = ecl_process_env();
#line 419
...
#line 465
	...
	newn = ecl_merge_pathnames(newn, oldn, ECL_SYM(":NEWEST",1276));
	new_filename = si_coerce_to_filename(newn);

	while (if_exists == ECL_SYM(":ERROR",1229) || if_exists == ECL_NIL)
        {
		...
		if (if_exists == ECL_SYM(":ERROR",1229)) {
			...
			if_exists =
				...
				(6, ECL_SYM("FILE-ERROR",353), /* condition */
				 ECL_SYM(":SUPERSEDE",1314), /* continuable */
				 /* format */
				 ecl_make_constant_base_string(msg,strlen(msg)),
				 cl_list(2, oldn, new_filename), /* format args */
				 ECL_SYM(":PATHNAME",1285), /* file-error options */
				 new_filename);
			if (if_exists == ECL_T) if_exists= ECL_SYM(":ERROR",1229);
		}
...
#line 498
	...
	if (ecl_unlikely(if_exists != ECL_SYM(":SUPERSEDE",1314) && if_exists != ECL_T)) {
		/* invalid key */
...
{
	if (pattern == ECL_NIL || pattern == ECL_SYM(":WILD",1327)) {
		return 1;
	} else {
...
dir_recursive(cl_object base_dir, cl_object directory, cl_object filemask, int flags)
{
 ...
 AGAIN:
	...
	if (item == ECL_SYM(":WILD",1327) || ecl_wild_string_p(item)) {
		/*
	...
	} else if (item == ECL_SYM(":WILD-INFERIORS",1328)) {
		/*
...
#line 955
	const cl_env_ptr the_env = ecl_process_env();
#line 955
	static cl_object KEYS[1] = {(cl_object)(cl_symbols+1837)};
...
si_get_library_pathname(void)
{
	...
	s->base_string.fillp = len;
        ...
        s = cl_make_pathname(8, ECL_SYM(":NAME",1273), ECL_NIL, ECL_SYM(":TYPE",1318), ECL_NIL,
			     ECL_SYM(":VERSION",1326), ECL_NIL,
                             ECL_SYM(":DEFAULTS",1216), s);
...
#line 1031
	...
	if (safe_chdir((char*)namestring->base_string.self, ECL_NIL) < 0) {
		...
		ECL_SETQ(the_env, ECL_SYM("*DEFAULT-PATHNAME-DEFAULTS*",32), directory);
	}
	{
#line 1054
		#line 1054
...
si_mkdir(cl_object directory, cl_object mode)
{
	...
	cl_object filename = si_coerce_to_base_string(directory);
        ...
        if (ecl_unlikely(!ECL_FIXNUMP(mode) ||
                ...
                FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::MKDIR*/1110), 2, mode,
...
si_mkstemp(cl_object template)
{
	...
	dir = cl_make_pathname(8,
	                       ECL_SYM(":TYPE",1318), ECL_NIL,
	                       ECL_SYM(":NAME",1273), ECL_NIL,
	                       ECL_SYM(":VERSION",1326), ECL_NIL,
	                       ECL_SYM(":DEFAULTS",1216), phys);
...
{
        return cl_delete_file(cl_make_pathname(6, ECL_SYM(":NAME",1273), ECL_NIL,
                                               ECL_SYM(":TYPE",1318), ECL_NIL,
                                               ECL_SYM(":DEFAULTS",1216), directory));
}
