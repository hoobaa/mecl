#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/file.d"
...
clos_stream_read_byte8(cl_object strm, unsigned char *c, cl_index n)
{
	cl_index i;
	for (i = 0; i < n; i++) {
		cl_object byte = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1636), strm);
		if (!ECL_FIXNUMP(byte))
			break;
		c[i] = ecl_fixnum(byte);
	}
...
clos_stream_write_byte8(cl_object strm, unsigned char *c, cl_index n)
{
	cl_index i;
	for (i = 0; i < n; i++) {
		cl_object byte = _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1644), strm,
					 ecl_make_fixnum(c[i]));
		if (!ECL_FIXNUMP(byte))
			break;
	}
...
{
	cl_object b = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1636), strm);
        if (b == ECL_SYM(":EOF",1228)) b = ECL_NIL;
        return b;
}
...
{
	_ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1644), strm, c);
}

static ecl_character
clos_stream_read_char(cl_object strm)
{
	cl_object output = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-CHAR",1637), strm);
        cl_fixnum value;
	if (ECL_CHARACTERP(output))
                value = ECL_CHAR_CODE(output);
        else if (ECL_FIXNUMP(output))
                value = ecl_fixnum(output);
	else if (output == ECL_NIL || output == ECL_SYM(":EOF",1228))
		return EOF;
        else
                value = -1;
...
{
	_ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-CHAR",1645), strm, ECL_CODE_CHAR(c));
	return c;
}
...
{
	_ecl_funcall3(ECL_SYM("GRAY::STREAM-UNREAD-CHAR",1643), strm, ECL_CODE_CHAR(c));
}

static int
clos_stream_peek_char(cl_object strm)
{
	cl_object out = _ecl_funcall2(ECL_SYM("GRAY::STREAM-PEEK-CHAR",1635), strm);
	if (out == ECL_SYM(":EOF",1228)) return EOF;
...
{
	return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-LISTEN",1634), strm));
}

static void
clos_stream_clear_input(cl_object strm)
{
	_ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-INPUT",1624), strm);
}

static void
clos_stream_clear_output(cl_object strm)
{
	_ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-OUTPUT",1625), strm);
	return;
}
...
{
	_ecl_funcall2(ECL_SYM("GRAY::STREAM-FORCE-OUTPUT",1630), strm);
}

static void
clos_stream_finish_output(cl_object strm)
{
	_ecl_funcall2(ECL_SYM("GRAY::STREAM-FINISH-OUTPUT",1629), strm);
}

static int
clos_stream_input_p(cl_object strm)
{
	return !Null(_ecl_funcall2(ECL_SYM("GRAY::INPUT-STREAM-P",1620), strm));
}

static int
clos_stream_output_p(cl_object strm)
{
	return !Null(_ecl_funcall2(ECL_SYM("GRAY::OUTPUT-STREAM-P",1621), strm));
}

static int
clos_stream_interactive_p(cl_object strm)
{
	return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-INTERACTIVE-P",1632), strm));
...
{
	return _ecl_funcall2(ECL_SYM("GRAY::STREAM-ELEMENT-TYPE",1626), strm);
}
...
{
	return _ecl_funcall2(ECL_SYM("GRAY::STREAM-FILE-POSITION",1628), strm);
}

static cl_object
clos_stream_set_position(cl_object strm, cl_object pos)
{
	return _ecl_funcall3(ECL_SYM("GRAY::STREAM-FILE-POSITION",1628), strm, pos);
}

static int
clos_stream_column(cl_object strm)
{
	cl_object col = _ecl_funcall2(ECL_SYM("GRAY::STREAM-LINE-COLUMN",1633), strm);
...
{
	return _ecl_funcall2(ECL_SYM("GRAY::CLOSE",1618), strm);
}
...
#line 1471
	if (element_type == ECL_SYM("BASE-CHAR",120)) {
		...
		extended = 1;
#endif
	} else if (!Null(_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, ECL_SYM("BASE-CHAR",120)))) {
		(void)0;
	} else if (!Null(_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, ECL_SYM("CHARACTER",222)))) {
#ifdef ECL_UNICODE
		extended = 1;
#endif
	} else {
...
cl_get_output_stream_string(cl_object strm)
{
	...
	unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_string_output))
                FEwrong_type_only_arg(ecl_make_fixnum(/*GET-OUTPUT-STREAM-STRING*/408),
                                      strm, ecl_make_fixnum(/*STRING-STREAM*/816));
...
{
	if (strm->stream.flags & ECL_STREAM_CLOSE_COMPONENTS) {
		cl_mapc(2, ECL_SYM("CLOSE",228), BROADCAST_STREAM_LIST(strm));
	}
	return generic_close(strm);
}
...
#line 1998
	...
	x = alloc_stream();
	x->stream.format = ECL_SYM(":DEFAULT",1215);
...
cl_broadcast_stream_streams(cl_object strm)
{
	unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_broadcast))
		FEwrong_type_only_arg(ecl_make_fixnum(/*BROADCAST-STREAM-STREAMS*/160),
                                      strm, ecl_make_fixnum(/*BROADCAST-STREAM*/159));
...
{
	if (strm->stream.flags & ECL_STREAM_CLOSE_COMPONENTS) {
		cl_mapc(2, ECL_SYM("CLOSE",228), CONCATENATED_STREAM_LIST(strm));
	}
	return generic_close(strm);
}
...
#line 2329
	...
	x = alloc_stream();
	if (Null(streams)) {
		x->stream.format = ECL_SYM(":PASS-THROUGH",1769);
	} else {
...
cl_concatenated_stream_streams(cl_object strm)
{
	unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_concatenated))
		FEwrong_type_only_arg(ecl_make_fixnum(/*CONCATENATED-STREAM-STREAMS*/246),
                                      strm, ecl_make_fixnum(/*CONCATENATED-STREAM*/245));
...
cl_make_synonym_stream(cl_object sym)
{
	...
	sym = ecl_check_cl_type(ECL_SYM("MAKE-SYNONYM-STREAM",538),sym,t_symbol);
	x = alloc_stream();
...
{
        if (format == ECL_SYM(":DEFAULT",1215)) {
                format = ecl_symbol_value(ECL_SYM("EXT::*DEFAULT-EXTERNAL-FORMAT*",1768));
        }
	if (CONSP(format)) {
	...
	if (format == ECL_SYM(":CR",1690)) {
		return (flags | ECL_STREAM_CR) & ~ECL_STREAM_LF;
	}
	if (format == ECL_SYM(":LF",1691)) {
		return (flags | ECL_STREAM_LF) & ~ECL_STREAM_CR;
	}
	if (format == ECL_SYM(":CRLF",1692)) {
		return flags | (ECL_STREAM_CR+ECL_STREAM_LF);
	}
	if (format == ECL_SYM(":LITTLE-ENDIAN",1701)) {
		return flags | ECL_STREAM_LITTLE_ENDIAN;
	}
	if (format == ECL_SYM(":BIG-ENDIAN",1702)) {
		return flags & ~ECL_STREAM_LITTLE_ENDIAN;
	}
	if (format == ECL_SYM(":PASS-THROUGH",1769)) {
#ifdef ECL_UNICODE
 ...
 PARSE_SYMBOLS:
	...
	if (ECL_SYMBOLP(format)) {
		format = _ecl_funcall2(ECL_SYM("EXT::MAKE-ENCODING",1698), format);
		if (ECL_SYMBOLP(format))
			goto PARSE_SYMBOLS;
		stream->stream.format_table = format;
		return (flags & ~ECL_STREAM_FORMAT) | ECL_STREAM_USER_FORMAT;
		    ...
		    cl_object external_format)
{
	...
	if (byte_size < 0) {
		...
		t = ECL_SYM("SIGNED-BYTE",761);
	} else {
		flags &= ~ECL_STREAM_SIGNED_BYTES;
		t = ECL_SYM("UNSIGNED-BYTE",885);
	}
	...
	case ECL_STREAM_LATIN_1:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",120);
		byte_size = 8;
		stream->stream.format = ECL_SYM(":LATIN-1",1681);
		stream->stream.encoder = passthrough_encoder;
		stream->stream.decoder = passthrough_decoder;
		break;
	case ECL_STREAM_UTF_8:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",222);
		byte_size = 8;
		stream->stream.format = ECL_SYM(":UTF-8",1683);
	...
	case ECL_STREAM_UCS_2:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",222);
		byte_size = 8*2;
		stream->stream.format = ECL_SYM(":UCS-2",1684);
	...
	case ECL_STREAM_UCS_2BE:
		...
		if (flags & ECL_STREAM_LITTLE_ENDIAN) {
			stream->stream.format = ECL_SYM(":UCS-2LE",1695);
			stream->stream.encoder = ucs_2le_encoder;
			stream->stream.decoder = ucs_2le_decoder;
		} else {
			stream->stream.format = ECL_SYM(":UCS-2BE",1693);
			stream->stream.encoder = ucs_2be_encoder;
			stream->stream.decoder = ucs_2be_decoder;
		}
		break;
	case ECL_STREAM_UCS_4:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",222);
		byte_size = 8*4;
		stream->stream.format = ECL_SYM(":UCS-4BE",1694);
	...
	case ECL_STREAM_UCS_4BE:
		...
		byte_size = 8*4;
		if (flags & ECL_STREAM_LITTLE_ENDIAN) {
			stream->stream.format = ECL_SYM(":UCS-4LE",1696);
			stream->stream.encoder = ucs_4le_encoder;
			stream->stream.decoder = ucs_4le_decoder;
		} else {
			stream->stream.format = ECL_SYM(":UCS-4BE",1694);
	...
	case ECL_STREAM_US_ASCII:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",120);
		byte_size = 8;
		stream->stream.format = ECL_SYM(":US-ASCII",1700);
	...
	case ECL_STREAM_DEFAULT_FORMAT:
		IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",120);
		byte_size = 8;
		stream->stream.format = ECL_SYM(":PASS-THROUGH",1769);
		stream->stream.encoder = passthrough_encoder;
		stream->stream.decoder = passthrough_decoder;
		break;
#endif
...
si_stream_external_format_set(cl_object stream, cl_object format)
{
	...
	case ecl_smm_io_wcon:
#endif
                {
                        ...
                        unlikely_if (elt_type != ECL_SYM("CHARACTER",222) &&
                                     elt_type != ECL_SYM("BASE-CHAR",120))
...
si_set_buffering_mode(cl_object stream, cl_object buffer_mode_symbol)
{
	...
	if (buffer_mode_symbol == ECL_SYM(":NONE",1600) || Null(buffer_mode_symbol))
		buffer_mode = _IONBF;
	else if (buffer_mode_symbol == ECL_SYM(":LINE",1603) || buffer_mode_symbol == ECL_SYM(":LINE-BUFFERED",1601))
		buffer_mode = _IOLBF;
	else if (buffer_mode_symbol == ECL_SYM(":FULL",1604) || buffer_mode_symbol == ECL_SYM(":FULLY-BUFFERED",1602))
		buffer_mode = _IOFBF;
	else
...
# ifdef WORDS_BIGENDIAN
                                external_format = ECL_SYM(":UCS-4BE",1694);
# else
                                external_format = ECL_SYM(":UCS-4LE",1696);
# endif
                        }
                }
#else
...
#line 4361
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(1928));
#line 4361
...
#line 4361
        p = ecl_vector_start_end(ecl_make_fixnum(/*EXT::MAKE-SEQUENCE-INPUT-STREAM*/1928),
                                 vector, start, end);
	{
#line 4364
		#line 4364
 ...
 AGAIN:
	...
	cl_fixnum last = vector->vector.dim;
        ...
        if (delta < n) {
                ...
                vector = _ecl_funcall3(ECL_SYM("ADJUST-ARRAY",83), vector,
...
#line 4503
	...
	ecl_va_start(ARGS, vector, narg, 1);
#line 4503
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(1929));
#line 4503
	cl_parse_key(ARGS, 1, KEYS, KEY_VARS, NULL, 0);
#line 4503
	if (KEY_VARS[1]==ECL_NIL) {
...
#endif
	unlikely_if (!ECL_ANSI_STREAM_P(stream)) {
                FEwrong_type_only_arg(ecl_make_fixnum(/*FILE-STRING-LENGTH*/359), stream, ecl_make_fixnum(/*STREAM*/799));
	}
...
si_do_write_sequence(cl_object seq, cl_object stream, cl_object s, cl_object e)
{
	...
	limit = ecl_length(seq);
        if (ecl_unlikely(!ECL_FIXNUMP(s) ||
                         ...
                         (start > limit))) {
                FEwrong_type_key_arg(ecl_make_fixnum(/*WRITE-SEQUENCE*/921), ecl_make_fixnum(/*:START*/1310), s,
	...
	if (e == ECL_NIL) {
                                ...
                                ((end = ecl_fixnum(e)) < 0) ||
                                (end > limit))) {
                FEwrong_type_key_arg(ecl_make_fixnum(/*WRITE-SEQUENCE*/921), ecl_make_fixnum(/*:END*/1225), e,
                                     ecl_make_integer_type(ecl_make_fixnum(0),
	...
	if (LISTP(seq)) {
		...
		bool ischar = (elt_type == ECL_SYM("BASE-CHAR",120)) || (elt_type == ECL_SYM("CHARACTER",222));
...
si_do_read_sequence(cl_object seq, cl_object stream, cl_object s, cl_object e)
{
	...
	limit = ecl_length(seq);
        if (ecl_unlikely(!ECL_FIXNUMP(s) ||
                         ((start = ecl_fixnum(s)) < 0) ||
                         (start > limit))) {
                FEwrong_type_key_arg(ecl_make_fixnum(/*READ-SEQUENCE*/698), ecl_make_fixnum(/*:START*/1310), s,
                                     ecl_make_integer_type(ecl_make_fixnum(0),
                                                           ecl_make_fixnum(limit-1)));
        }
	if (e == ECL_NIL) {
                                ...
                                ((end = ecl_fixnum(e)) < 0) ||
                                (end > limit))) {
                FEwrong_type_key_arg(ecl_make_fixnum(/*READ-SEQUENCE*/698), ecl_make_fixnum(/*:END*/1225), e,
                                     ecl_make_integer_type(ecl_make_fixnum(0),
                                                           ecl_make_fixnum(limit)));
        }
	if (end <= start) {
...
#line 4885
	if (Null(position)) {
		...
		if (position == ECL_SYM(":START",1310)) {
			position = ecl_make_fixnum(0);
		} else if (position == ECL_SYM(":END",1225)) {
			position = ECL_NIL;
		}
...
#ifdef ECL_CLOS_STREAMS
	if (ECL_INSTANCEP(strm)) {
		return _ecl_funcall2(ECL_SYM("GRAY::OPEN-STREAM-P",1622), strm);
	}
#endif
...
#ifdef ECL_CLOS_STREAMS
	if (t == t_instance)
		output = ECL_SYM(":DEFAULT",1215);
	else
#endif
        unlikely_if (t != t_stream)
                FEwrong_type_only_arg(ecl_make_fixnum(/*STREAM-EXTERNAL-FORMAT*/803), strm, ecl_make_fixnum(/*STREAM*/799));
...
#ifdef ECL_CLOS_STREAMS
	if (ECL_INSTANCEP(strm)) {
		return _ecl_funcall2(ECL_SYM("GRAY::STREAMP",1619), strm);
	}
#endif
	{
#line 4968
...
ecl_normalize_stream_element_type(cl_object element_type)
{
	cl_fixnum sign = 0;
	cl_index size;
	if (element_type == ECL_SYM("SIGNED-BYTE",761) || element_type == ECL_SYM("EXT::INTEGER8",1335)) {
		return -8;
	} else if (element_type == ECL_SYM("UNSIGNED-BYTE",885) || element_type == ECL_SYM("EXT::BYTE8",1330)) {
		return 8;
	} else if (element_type == ECL_SYM(":DEFAULT",1215)) {
		return 0;
        } else if (element_type == ECL_SYM("BASE-CHAR",120) || element_type == ECL_SYM("CHARACTER",222)) {
                return 0;
	} else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, ECL_SYM("CHARACTER",222)) != ECL_NIL) {
		return 0;
	} else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, ECL_SYM("UNSIGNED-BYTE",885)) != ECL_NIL) {
		sign = +1;
	} else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, ECL_SYM("SIGNED-BYTE",761)) != ECL_NIL) {
		sign = -1;
	} else {
		FEerror("Not a valid stream element type: ~A", 1, element_type);
	}
	if (CONSP(element_type)) {
		if (CAR(element_type) == ECL_SYM("UNSIGNED-BYTE",885))
			return ecl_to_size(cl_cadr(element_type));
		if (CAR(element_type) == ECL_SYM("SIGNED-BYTE",761))
			return -ecl_to_size(cl_cadr(element_type));
	}
	for (size = 8; 1; size++) {
		cl_object type;
		type = cl_list(2, sign>0? ECL_SYM("UNSIGNED-BYTE",885) : ECL_SYM("SIGNED-BYTE",761),
			       ecl_make_fixnum(size));
		if (_ecl_funcall3(ECL_SYM("SUBTYPEP",837), element_type, type) != ECL_NIL) {
			return size * sign;
		}
	}
	FEerror("Not a valid stream element type: ~A", 1, element_type);
		...
		int flags, cl_object external_format)
{
	...
	if (smm == ecl_smm_input || smm == ecl_smm_probe) {
		if (!exists) {
			if (if_does_not_exist == ECL_SYM(":ERROR",1229)) {
				FEcannot_open(fn);
			} else if (if_does_not_exist == ECL_SYM(":CREATE",1213)) {
				...
				FEinvalid_option(ECL_SYM(":IF-DOES-NOT-EXIST",1244),
						 if_does_not_exist);
			}
		}
		...
		if (if_exists == ECL_SYM(":NEW-VERSION",1275) &&
		    if_does_not_exist == ECL_SYM(":CREATE",1213)) {
			exists = 0;
			if_does_not_exist = ECL_SYM(":CREATE",1213);
		}
		if (exists) {
			if (if_exists == ECL_SYM(":ERROR",1229)) {
				FEcannot_open(fn);
			} else if (if_exists == ECL_SYM(":RENAME",1300)) {
				f = ecl_backup_open(fname, base|O_CREAT, mode);
				unlikely_if (f < 0) FEcannot_open(fn);
			} else if (if_exists == ECL_SYM(":RENAME-AND-DELETE",1301) ||
				   if_exists == ECL_SYM(":NEW-VERSION",1275) ||
				   if_exists == ECL_SYM(":SUPERSEDE",1314)) {
				f = safe_open(fname, base|O_TRUNC, mode);
				unlikely_if (f < 0) FEcannot_open(fn);
			} else if (if_exists == ECL_SYM(":OVERWRITE",1283) || if_exists == ECL_SYM(":APPEND",1198)) {
				f = safe_open(fname, base, mode);
				unlikely_if (f < 0) FEcannot_open(fn);
				appending = (if_exists == ECL_SYM(":APPEND",1198));
			} else if (Null(if_exists)) {
				return ECL_NIL;
			} else {
				FEinvalid_option(ECL_SYM(":IF-EXISTS",1245), if_exists);
			}
		} else {
			if (if_does_not_exist == ECL_SYM(":ERROR",1229)) {
				FEcannot_open(fn);
	...
	if (flags & ECL_STREAM_C_STREAM) {
		...
		output = ecl_make_stream_from_FILE(fn, fp, smm, byte_size, flags,
						   external_format);
		si_set_buffering_mode(output, byte_size? ECL_SYM(":FULL",1604) : ECL_SYM(":LINE",1603));
	} else {
...
#line 5156
	...
	if (direction == ECL_SYM(":INPUT",1254)) {
		...
		if (!iesp)
			if_exists = ECL_SYM(":NEW-VERSION",1275);
		if (!idnesp) {
			if (if_exists == ECL_SYM(":OVERWRITE",1283) ||
			    if_exists == ECL_SYM(":APPEND",1198))
				if_does_not_exist = ECL_SYM(":ERROR",1229);
			else
				if_does_not_exist = ECL_SYM(":CREATE",1213);
		}
	...
	} else if (direction == ECL_SYM(":PROBE",1293)) {
		smm = ecl_smm_probe;
		if (!idnesp)
			if_does_not_exist = ECL_NIL;
	} else {
...
# if defined(HAVE_SELECT)
	...
	if (ecl_unlikely(retv < 0))
		file_libc_error(ecl_make_fixnum(/*STREAM-ERROR*/801), stream, "Error while listening to stream.", 0);
	else if (retv > 0)
		return ECL_LISTEN_AVAILABLE;
	else
		return ECL_LISTEN_NO_CHAR;
...
flisten(cl_object stream, FILE *fp)
{
	   ...
	   It will fail on noninteractive streams. */
	{
		...
		unlikely_if (ecl_fseeko(fp, 0, SEEK_END) != 0)
			file_libc_error(ecl_make_fixnum(/*FILE-ERROR*/353), stream,
...
{
	return cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",773), ECL_SYM(":FORMAT-CONTROL",1240),
			make_constant_base_string("~A is not an file stream"),
			ECL_SYM(":FORMAT-ARGUMENTS",1239), cl_list(1, strm),
			ECL_SYM(":EXPECTED-TYPE",1232), ECL_SYM("FILE-STREAM",358),
			ECL_SYM(":DATUM",1214), strm);
}
...
not_an_input_stream(cl_object strm)
{
	cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",773), ECL_SYM(":FORMAT-CONTROL",1240),
		 ...
		 ECL_SYM(":FORMAT-ARGUMENTS",1239), cl_list(1, strm),
		 ECL_SYM(":EXPECTED-TYPE",1232),
		 cl_list(2, ECL_SYM("SATISFIES",735), ECL_SYM("INPUT-STREAM-P",435)),
		 ECL_SYM(":DATUM",1214), strm);
}
...
not_an_output_stream(cl_object strm)
{
	cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",773), ECL_SYM(":FORMAT-CONTROL",1240),
		 ...
		 ECL_SYM(":FORMAT-ARGUMENTS",1239), cl_list(1, strm),
		 ECL_SYM(":EXPECTED-TYPE",1232), cl_list(2, ECL_SYM("SATISFIES",735), ECL_SYM("OUTPUT-STREAM-P",616)),
		 ECL_SYM(":DATUM",1214), strm);
}
...
not_a_character_stream(cl_object s)
{
	cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",773), ECL_SYM(":FORMAT-CONTROL",1240),
		 ...
		 ECL_SYM(":FORMAT-ARGUMENTS",1239), cl_list(1, s),
		 ECL_SYM(":EXPECTED-TYPE",1232), ECL_SYM("CHARACTER",222),
		 ECL_SYM(":DATUM",1214), cl_stream_element_type(s));
}
...
not_a_binary_stream(cl_object s)
{
	cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",773), ECL_SYM(":FORMAT-CONTROL",1240),
		 ...
		 ECL_SYM(":FORMAT-ARGUMENTS",1239), cl_list(1, s),
		 ECL_SYM(":EXPECTED-TYPE",1232), ECL_SYM("INTEGER",437),
		 ECL_SYM(":DATUM",1214), cl_stream_element_type(s));
}
...
{
	file_libc_error(ecl_make_fixnum(/*FILE-ERROR*/353), stream, "Stream cannot be closed", 0);
}
...
restartable_io_error(cl_object strm, const char *s)
{
	...
	if (old_errno == EINTR) {
		...
		file_libc_error(ecl_make_fixnum(/*STREAM-ERROR*/801), strm,
...
{
        cl_object code = _ecl_funcall4(ECL_SYM("EXT::ENCODING-ERROR",1831), stream,
...
decoding_error(cl_object stream, unsigned char *buffer, int length)
{
        ...
        code = _ecl_funcall4(ECL_SYM("EXT::DECODING-ERROR",1830), stream,
			    cl_stream_external_format(stream),
			    octets);
        if (Null(code)) {
...
#if defined(ECL_MS_WINDOWS_HOST)
# ifdef ECL_UNICODE
	external_format = cl_list(2, ECL_SYM(":LATIN-1",1681), ECL_SYM(":CRLF",1692));
	flags = 0;
# else
	external_format = cl_list(2, ECL_SYM(":CRLF",1692), ECL_SYM(":PASS-THROUGH",1769));
...
# endif
	...
	cl_core.standard_input = standard_input;
        ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-INPUT+",1811), standard_input);
	ECL_SET(ECL_SYM("*STANDARD-INPUT*",66), standard_input);
	cl_core.standard_output = standard_output;
        ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-OUTPUT+",1812), standard_output);
	ECL_SET(ECL_SYM("*STANDARD-OUTPUT*",67), standard_output);
	ECL_SET(ECL_SYM("*TRACE-OUTPUT*",69), standard_output);
	cl_core.error_output = error_output;
        ECL_SET(ECL_SYM("EXT::+PROCESS-ERROR-OUTPUT+",1813), error_output);
	ECL_SET(ECL_SYM("*ERROR-OUTPUT*",33), error_output);

	cl_core.terminal_io = aux 
		= cl_make_two_way_stream(standard_input, standard_output);

	ECL_SET(ECL_SYM("*TERMINAL-IO*",68), aux);
	aux = cl_make_synonym_stream(ECL_SYM("*TERMINAL-IO*",68));
	ECL_SET(ECL_SYM("*QUERY-IO*",59), aux);
	ECL_SET(ECL_SYM("*DEBUG-IO*",30), aux);
}
