#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/disassembler.d"
...
disassemble_lambda(cl_object bytecodes) {
	...
	ecl_bds_bind(env, ECL_SYM("*PRINT-PRETTY*",55), ECL_NIL);
...
disassemble(cl_object bytecodes, cl_opcode *vector) {
	...
	if (cl_fboundp(ECL_SYM("SI::FORMATTER-AUX",1075)) != ECL_NIL)
...
si_bc_join(cl_object lex, cl_object code, cl_object data, cl_object name)
{
        ...
        if (lex != ECL_NIL) {
                ...
                unlikely_if (!ECL_VECTORP(code) ||
                        ...
                        FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::BC-JOIN*/1046),
                                             0, code,
                                             cl_list(2,
                                                     ECL_SYM("SIMPLE-ARRAY",763),
                                                     ECL_SYM("EXT::BYTE8",1330)));
                }
                unlikely_if (!ECL_VECTORP(code) ||
