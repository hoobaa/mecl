#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/main.d"
...
cl_shutdown(void)
{
	if (ecl_option_values[ECL_OPT_BOOTED] > 0) {
		cl_object l = ecl_symbol_value(ECL_SYM("SI::*EXIT-HOOKS*",1473));
		cl_object form = cl_list(2, ECL_SYM("FUNCALL",394), ECL_NIL);
		while (CONSP(l)) {
			ecl_elt_set(form, 1, ECL_CONS_CAR(l));
			si_safe_eval(3, form, ECL_NIL, OBJNULL);
			l = CDR(l);
			ECL_SET(ECL_SYM("SI::*EXIT-HOOKS*",1473), l);
		}
#ifdef ENABLE_DLOPEN
...
cl_boot(int argc, char **argv)
{
	...
	GC_enable();
         ...
         */
#if 1
	ECL_SET(ECL_SYM("*DEFAULT-PATHNAME-DEFAULTS*",32), si_getcwd(0));
#else
	ECL_SET(ECL_SYM("*DEFAULT-PATHNAME-DEFAULTS*",32),
		ecl_make_pathname(ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_SYM(":LOCAL",1268)));
#endif
	...
	ECL_SET(ECL_SYM("MP::*CURRENT-PROCESS*",1399), env->own_process);
#endif
	...
	cl_core.char_names = aux =
	    cl__make_hash_table(ECL_SYM("EQUALP",336), ecl_make_fixnum(128), /* size */
				cl_core.rehash_size,
	...
	cl_core.system_properties =
	    cl__make_hash_table(ECL_SYM("EQUAL",335), ecl_make_fixnum(1024), /* size */
				cl_core.rehash_size,
                                cl_core.rehash_threshold);
	cl_core.setf_definitions =
	    cl__make_hash_table(ECL_SYM("EQ",333), ecl_make_fixnum(256), /* size */
				cl_core.rehash_size,
                                cl_core.rehash_threshold);

	ECL_SET(ECL_SYM("*RANDOM-STATE*",60), ecl_make_random_state(ECL_T));

	ECL_SET(ECL_SYM("FFI::C-INT-MAX",1958), ecl_make_integer(INT_MAX));
	ECL_SET(ECL_SYM("FFI::C-INT-MIN",1959), ecl_make_integer(INT_MIN));
	ECL_SET(ECL_SYM("FFI::C-LONG-MAX",1962), ecl_make_integer(LONG_MAX));
	ECL_SET(ECL_SYM("FFI::C-LONG-MIN",1963), ecl_make_integer(LONG_MIN));
	ECL_SET(ECL_SYM("FFI::C-UINT-MAX",1966), ecl_make_unsigned_integer(UINT_MAX));
	ECL_SET(ECL_SYM("FFI::C-ULONG-MAX",1968), ecl_make_unsigned_integer(ULONG_MAX));
#ifdef ecl_long_long_t
	ECL_SET(ECL_SYM("FFI::C-LONG-LONG-MAX",1964), ecl_make_unsigned_integer(LLONG_MAX));
	ECL_SET(ECL_SYM("FFI::C-ULONG-LONG-MAX",1969), ecl_make_unsigned_integer(ULLONG_MAX));
#endif
	...
	ECL_SET(ECL_SYM("*PRINT-CASE*",46), ECL_SYM(":UPCASE",1322));

	/*
	 * Set up hooks for LOAD, errors and macros.
	 */
#ifdef ECL_THREADS
	ECL_SET(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1427),
		ecl_make_lock(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1427), 1));
#endif
	aux = cl_list(
#ifdef ENABLE_DLOPEN
		11,
                CONS(str_fas, ECL_SYM("SI::LOAD-BINARY",1589)),
		CONS(str_fasl, ECL_SYM("SI::LOAD-BINARY",1589)),
		CONS(str_fasb, ECL_SYM("SI::LOAD-BINARY",1589)),
		CONS(str_FASB, ECL_SYM("SI::LOAD-BINARY",1589)),
#else
		7,
#endif
		CONS(str_lsp, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_lisp, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_LSP, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_LISP, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_fasc, ECL_SYM("SI::LOAD-BYTECODES",1838)),
		CONS(str_FASC, ECL_SYM("SI::LOAD-BYTECODES",1838)),
		CONS(ECL_NIL, ECL_SYM("SI::LOAD-SOURCE",1099)));
	ECL_SET(ECL_SYM("EXT::*LOAD-HOOKS*",1025), aux);
	init_error();
	init_macros();
	init_compiler();
	 ...
	 */
#ifdef CLOS
	ECL_SET(ECL_SYM("SI::*CLASS-NAME-HASH-TABLE*",1016),
		cl__make_hash_table(ECL_SYM("EQ",333), ecl_make_fixnum(1024), /* size */
                                    cl_core.rehash_size,
                                    cl_core.rehash_threshold));
#endif
	...
	ECL_SET(ECL_SYM("LAMBDA-LIST-KEYWORDS",453),
		cl_list(8, ECL_SYM("&OPTIONAL",11), ECL_SYM("&REST",12), ECL_SYM("&KEY",10), ECL_SYM("&ALLOW-OTHER-KEYS",6),
			ECL_SYM("&AUX",7), ECL_SYM("&WHOLE",13), ECL_SYM("&ENVIRONMENT",9), ECL_SYM("&BODY",8)));
	...
	ECL_SET(ECL_SYM("*PACKAGE*",43), cl_core.lisp_package);
	...
	if (cl_fboundp(ECL_SYM("EXT::MAKE-ENCODING",1698)) != ECL_NIL) {
	...
	/* Jump to top level */
	ECL_SET(ECL_SYM("*PACKAGE*",43), cl_core.user_package);
	init_unixint(1);
	return 1;
}
...
#endif
        ECL_SET(ECL_SYM("EXT::*PROGRAM-EXIT-CODE*",1741), code);
...
#line 833
	...
	va_start(ARGS, narg);
#line 833
	if (ecl_unlikely(narg < 0|| narg > 1)) FEwrong_num_arguments(ecl_make_fixnum(1742));
#line 833
	if (narg > 0) {
#line 833
		code = va_arg(ARGS,cl_object);
...
#line 833
		code = ECL_SYM_VAL(ecl_process_env(),ECL_SYM("EXT::*PROGRAM-EXIT-CODE*",1741));
#line 833
	}
#line 833
        cl_shutdown();
