...
si_sse_pack_as_elt_type(cl_object x, cl_object type)
{
	...
	if (ecl_unlikely(!ECL_SSE_PACK_P(x))) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*EXT::SSE-PACK-AS-ELT-TYPE*/1794), 1, x, ecl_make_fixnum(/*EXT::SSE-PACK*/1792));
	}
...
{
	if (ecl_unlikely(!ECL_SSE_PACK_P(x))) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*EXT::SSE-PACK-ELEMENT-TYPE*/1800), 1, x, ecl_make_fixnum(/*EXT::SSE-PACK*/1792));
	}
...
si_sse_pack_to_vector(cl_object x, cl_object elt_type)
{
	...
	if (ecl_unlikely(!ECL_SSE_PACK_P(x))) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*EXT::SSE-PACK-TO-VECTOR*/1796), 1, x, ecl_make_fixnum(/*EXT::SSE-PACK*/1792));
	}
...
si_vector_to_sse_pack(cl_object x)
{
	...
	if (ecl_unlikely(!ECL_ARRAYP(x))) {
                FEwrong_type_nth_arg(ecl_make_fixnum(/*EXT::VECTOR-TO-SSE-PACK*/1795), 1, x, ecl_make_fixnum(/*ARRAY*/96));
        }
...
ecl_unbox_int_sse_pack(cl_object x)
{
	do {
		...
		x = ecl_type_error(ECL_SYM("COERCE",231), "variable", x, ECL_SYM("EXT::SSE-PACK",1792));
	} while(1);
}
