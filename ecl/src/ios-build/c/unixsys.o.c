...
{
        return _ecl_funcall1(ECL_SYM("EXT::MAKE-EXTERNAL-PROCESS",1745));
}

static cl_object
external_process_pid(cl_object p)
{
        return ecl_structure_ref(p, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 0);
}

static cl_object
external_process_status(cl_object p)
{
        return ecl_structure_ref(p, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 3);
}

static cl_object
external_process_code(cl_object p)
{
        return ecl_structure_ref(p, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 4);
}

static void
set_external_process_pid(cl_object process, cl_object pid)
{
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 0, pid);
}

static void
set_external_process_streams(cl_object process, cl_object input, cl_object output)
{
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 1, input);
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 2, output);
}
...
{
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 0, ECL_NIL);
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 4, code);
        ecl_structure_set(process, ECL_SYM("EXT::EXTERNAL-PROCESS",1746), 3, status);
}
...
ecl_waitpid(cl_object pid, cl_object wait)
{
        ...
        if (!ok) {
                ...
                status = ECL_SYM(":RUNNING",1751);
                code = ECL_NIL;
        } else {
                status = ECL_SYM(":EXITED",1752);
...
#line 294
{
        ...
        do {
				...
				set_external_process_pid(p, ECL_NIL);
                        ...
                        if (status != ECL_SYM(":RUNNING",1751)) {
...
{
        cl_object foreign = ecl_allocate_foreign_data(ECL_SYM(":POINTER-VOID",1378),
						      sizeof(HANDLE*));
        HANDLE *ph = (HANDLE*)foreign->foreign.data;
        *ph = h;
        si_set_finalizer(foreign, ECL_SYM("SI::CLOSE-WINDOWS-HANDLE",1756));
        return foreign;
}
#endif
...
#line 344
	if (ecl_unlikely(narg < 1|| narg > 2)) FEwrong_num_arguments(ecl_make_fixnum(1755));
#line 344
	if (narg > 1) {
#line 344
...
#line 344
{
 ...
 AGAIN:
		...
		do {
			...
			ecl_musleep(0.0, 1);
		} while (status == ECL_SYM(":RUNNING",1751));
        } else {
...
#line 415
	const cl_env_ptr the_env = ecl_process_env();
#line 415
	static cl_object KEYS[6] = {(cl_object)(cl_symbols+1254), (cl_object)(cl_symbols+1282), (cl_object)(cl_symbols+1229), (cl_object)(cl_symbols+1329), (cl_object)(cl_symbols+1744), (cl_object)(cl_symbols+1246)};
	cl_object input;
	cl_object output;
	cl_object error;
	cl_object wait;
