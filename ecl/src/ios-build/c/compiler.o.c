...
asm_end(cl_env_ptr env, cl_index beginning, cl_object definition) {
	...
	bytecodes->bytecodes.name = ECL_SYM("SI::BYTECODES",1661);
...
static compiler_record database[] = {
  ...
  {ECL_SYM("EXT::TRULY-THE",1951), c_the, 0},
  {ECL_SYM("THROW",858), c_throw, 1},
  {ECL_SYM("UNWIND-PROTECT",888), c_unwind_protect, 1},
  {ECL_SYM("VALUES",895), c_values, 1},
  {ECL_SYM("SI::WHILE",1470), c_while, 0},
  {ECL_SYM("EXT::WITH-BACKEND",1770), c_with_backend, 0},
  {ECL_SYM("SI::UNTIL",1471), c_until, 0},
  ...
  {ECL_SYM("SI::CONS-CAR",1780), c_car, 1},
  {ECL_SYM("SI::CONS-CDR",1781), c_cdr, 1},
  {NULL, NULL, 1}
};
          ...
          cl_compiler_env_ptr old)
{
	...
	if (old) {
		*new = *old;
		...
		new->code_walker = ECL_SYM_VAL(the_env, ECL_SYM("SI::*CODE-WALKER*",1590));
...
c_var_ref(cl_env_ptr env, cl_object var, int allow_symbol_macro, bool ensure_defined)
{
	...
	if (ensure_defined) {
		l = ecl_symbol_value(ECL_SYM("EXT::*ACTION-ON-UNDEFINED-VARIABLE*",1598));
		if (l != ECL_NIL) {
...
c_macrolet(cl_env_ptr the_env, cl_object args, int flags)
{
	...
	cl_object env = funcall(3, ECL_SYM("SI::CMP-ENV-REGISTER-MACROLET",1605), pop(&args),
...
need_to_make_load_form_p(cl_object o)
{
        ...
        case t_list:
                if (Null(o)) return 0;
        default:
                return _ecl_funcall3(ECL_SYM("CLOS::NEED-TO-MAKE-LOAD-FORM-P",1579), o, ECL_NIL)
                        != ECL_NIL;
        }
}
...
compile_form(cl_env_ptr env, cl_object stmt, int flags) {
	...
	ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1614), stmt);
 BEGIN:
...
ecl_make_lambda(cl_env_ptr env, cl_object name, cl_object lambda) {
	...
	ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1614),
	...
	c_register_var(env, ECL_SYM("SI::FUNCTION-BOUNDARY",1835), TRUE, FALSE);
