...
clos_set_funcallable_instance_function(cl_object x, cl_object function_or_t)
{
	if (ecl_unlikely(!ECL_INSTANCEP(x)))
                FEwrong_type_nth_arg(ecl_make_fixnum(/*CLOS::SET-FUNCALLABLE-INSTANCE-FUNCTION*/1541),
	...
	} else if (function_or_t == ECL_SYM("CLOS::STANDARD-OPTIMIZED-READER-METHOD",1568)) {
	...
	} else if (function_or_t == ECL_SYM("CLOS::STANDARD-OPTIMIZED-WRITER-METHOD",1569)) {
		/* WARNING: We assume that f(a,...) behaves as f(a,b) */
...
generic_compute_applicable_method(cl_env_ptr env, cl_object frame, cl_object gf)
{
	/* method not cached */
	cl_object memoize;
	cl_object methods = _ecl_funcall3(ECL_SYM("CLOS::COMPUTE-APPLICABLE-METHODS-USING-CLASSES",1496),
