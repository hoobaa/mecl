...
#ifdef ECL_THREADS
	case t_process:
		return ECL_SYM("MP::PROCESS",1396);
	case t_lock:
		return ECL_SYM("MP::LOCK",1397);
	case t_condition_variable:
		return ECL_SYM("MP::CONDITION-VARIABLE",1398);
	case t_semaphore:
		return ECL_SYM("MP::SEMAPHORE",1444);
	case t_barrier:
		return ECL_SYM("MP::BARRIER",1452);
	case t_mailbox:
		return ECL_SYM("MP::MAILBOX",1463);
#endif
	case t_codeblock:
		return ECL_SYM("SI::CODE-BLOCK",1615);
	case t_foreign:
		return ECL_SYM("SI::FOREIGN-DATA",1345);
	case t_frame:
		return ECL_SYM("SI::FRAME",1616);
	case t_weak_pointer:
		return ECL_SYM("EXT::WEAK-POINTER",1729);
#ifdef ECL_SSE2
	case t_sse_pack:
		return ECL_SYM("EXT::SSE-PACK",1792);
#endif
	default:
...
cl_type_of(cl_object x)
{
	...
	case t_stream:
		...
		case ecl_smm_echo:		t = ECL_SYM("ECHO-STREAM",322); break;
                ...
                case ecl_smm_sequence_output: t = ECL_SYM("EXT::SEQUENCE-STREAM",1927); break;
