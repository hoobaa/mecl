#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/stacks.d"
...
ecl_cs_overflow(void)
{
	...
	cl_cerror(6, make_constant_base_string("Extend stack size"),
		  ECL_SYM("EXT::STACK-OVERFLOW",1667), ECL_SYM(":SIZE",1308), ecl_make_fixnum(size),
		  ECL_SYM(":TYPE",1318), ECL_SYM("EXT::C-STACK",1673));
	size += size / 2;
...
ecl_bds_overflow(void)
{
	...
	cl_cerror(6, make_constant_base_string("Extend stack size"),
		  ECL_SYM("EXT::STACK-OVERFLOW",1667), ECL_SYM(":SIZE",1308), ecl_make_fixnum(size),
		  ECL_SYM(":TYPE",1318), ECL_SYM("EXT::BINDING-STACK",1670));
...
#line 230
		#line 230
		cl_object __value0 = ((v == OBJNULL)? ECL_UNBOUND : v);
#line 230
		the_env->nvalues = 1;
#line 230
		return __value0;
...
frs_overflow(void)		/* used as condition in list.d */
{
	...
	cl_cerror(6, make_constant_base_string("Extend stack size"),
		  ECL_SYM("EXT::STACK-OVERFLOW",1667), ECL_SYM(":SIZE",1308), ecl_make_fixnum(size),
		  ECL_SYM(":TYPE",1318), ECL_SYM("EXT::FRAME-STACK",1671));
...
si_set_limit(cl_object type, cl_object size)
{
	...
	if (type == ECL_SYM("EXT::FRAME-STACK",1671)) {
		frs_set_size(env, the_size);
	} else if (type == ECL_SYM("EXT::BINDING-STACK",1670)) {
		ecl_bds_set_size(env, the_size);
	} else if (type == ECL_SYM("EXT::C-STACK",1673)) {
		cs_set_size(env, the_size);
	} else if (type == ECL_SYM("EXT::LISP-STACK",1672)) {
