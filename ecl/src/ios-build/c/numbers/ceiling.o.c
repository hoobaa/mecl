#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/numbers/ceiling.d"
...
#endif
	default:
		FEwrong_type_nth_arg(ecl_make_fixnum(/*CEILING*/199),1,x,ecl_make_fixnum(/*REAL*/703));
	}
	{
#line 74
		const cl_env_ptr the_env = ecl_process_env();
...
ecl_ceiling2(cl_object x, cl_object y)
{
	...
	if (ecl_unlikely(!ECL_REAL_TYPE_P(ty))) {
		FEwrong_type_nth_arg(ecl_make_fixnum(/*CEILING*/199),2, y, ecl_make_fixnum(/*REAL*/703));
	}
	switch(ecl_t_of(x)) {
	case t_fixnum:
		switch(ty) {
