...
ecl_read_object_non_recursive(cl_object in)
{
        ...
        nlogd(">>@ecl_read_object_non_recursive from. ansi_stream_p(%d)", ECL_ANSI_STREAM_P(in));
        x = ecl_read_object(in);
        nlogd(">>@ecl_read_object_non_recursive to");
                               ...
                               enum ecl_chattrib a)
{
 ...
 BEGIN:
        ...
        if ((a == cat_terminating || a == cat_non_terminating) && (flags != ECL_READ_ONLY_TOKEN)) {
                cl_object o;
                if (ECL_HASH_TABLE_P(x)) {
                        nlogd(">>");
                        o = dispatch_macro_character(x, in, c);
                } else {
                        nlogd(">> ansi_stream_p(%d)", ECL_ANSI_STREAM_P(in)); //////////////////////////////
