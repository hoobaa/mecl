#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/compiler.d"
...
asm_end(cl_env_ptr env, cl_index beginning, cl_object definition) {
	...
	cl_opcode *code;
        cl_object file = ECL_SYM_VAL(env,ECL_SYM("EXT::*SOURCE-LOCATION*",1180)), position;
        if (Null(file)) {
                file = ECL_SYM_VAL(env,ECL_SYM("*LOAD-TRUENAME*",38));
	...
	bytecodes->bytecodes.name = ECL_SYM("SI::BYTECODES",1659);
...
static compiler_record database[] = {
  {ECL_SYM("BLOCK",137), c_block, 1},
  {ECL_SYM("CASE",181), c_case, 1},
  {ECL_SYM("CATCH",182), c_catch, 1},
  {ECL_SYM("EXT::COMPILER-LET",1334), c_compiler_let, 0},
  {ECL_SYM("COND",247), c_cond, 1},
  {ECL_SYM("EVAL-WHEN",340), c_eval_when, 0},
  {ECL_SYM("FLET",373), c_flet, 1},
  {ECL_SYM("FUNCTION",396), c_function, 1},
  {ECL_SYM("FUNCALL",394), c_funcall, 1},
  {ECL_SYM("GO",414), c_go, 1},
  {ECL_SYM("IF",946), c_if, 1},
  {ECL_SYM("LABELS",451), c_labels, 1},
  {ECL_SYM("LET",477), c_let, 1},
  {ECL_SYM("LET*",478), c_leta, 1},
  {ECL_SYM("LOCALLY",490), c_locally, 0},
  {ECL_SYM("LOAD-TIME-VALUE",489), c_load_time_value, 1},
  {ECL_SYM("MACROLET",521), c_macrolet, 0},
  {ECL_SYM("MULTIPLE-VALUE-BIND",572), c_multiple_value_bind, 1},
  {ECL_SYM("MULTIPLE-VALUE-CALL",573), c_multiple_value_call, 1},
  {ECL_SYM("MULTIPLE-VALUE-PROG1",575), c_multiple_value_prog1, 1},
  {ECL_SYM("MULTIPLE-VALUE-SETQ",576), c_multiple_value_setq, 1},
  {ECL_SYM("NOT",584), c_not, 1},
  {ECL_SYM("NTH-VALUE",603), c_nth_value, 1},
  {ECL_SYM("NULL",605), c_not, 1},
  {ECL_SYM("PROGN",671), compile_toplevel_body, 0},
  {ECL_SYM("PROG1",669), c_prog1, 1},
  {ECL_SYM("PROGV",673), c_progv, 1},
  {ECL_SYM("PSETQ",676), c_psetq, 1},
  {ECL_SYM("QUOTE",679), c_quote, 1},
  {ECL_SYM("RETURN",724), c_return, 1},
  {ECL_SYM("RETURN-FROM",725), c_return_from, 1},
  {ECL_SYM("SETQ",751), c_setq, 1},
  {ECL_SYM("SYMBOL-MACROLET",842), c_symbol_macrolet, 0},
  {ECL_SYM("TAGBODY",850), c_tagbody, 1},
  {ECL_SYM("THE",856), c_the, 0},
  {ECL_SYM("EXT::TRULY-THE",1949), c_the, 0},
  {ECL_SYM("THROW",858), c_throw, 1},
  {ECL_SYM("UNWIND-PROTECT",888), c_unwind_protect, 1},
  {ECL_SYM("VALUES",895), c_values, 1},
  {ECL_SYM("SI::WHILE",1469), c_while, 0},
  {ECL_SYM("EXT::WITH-BACKEND",1768), c_with_backend, 0},
  {ECL_SYM("SI::UNTIL",1470), c_until, 0},

  /* Extras */

  {ECL_SYM("CONS",251), c_cons, 1},
  {ECL_SYM("CAR",180), c_car, 1},
  {ECL_SYM("CDR",198), c_cdr, 1},
  {ECL_SYM("FIRST",371), c_car, 1},
  {ECL_SYM("REST",719), c_cdr, 1},
  {ECL_SYM("LIST",481), c_list, 1},
  {ECL_SYM("LIST*",482), c_listA, 1},
  {ECL_SYM("ENDP",330), c_endp, 1},
  {ECL_SYM("SI::CONS-CAR",1778), c_car, 1},
  {ECL_SYM("SI::CONS-CDR",1779), c_cdr, 1},
  {NULL, NULL, 1}
};
...
c_register_block(cl_env_ptr env, cl_object name)
{
	...
	c_env->variables = CONS(cl_list(4, ECL_SYM(":BLOCK",1202), name, ECL_NIL, loc),
                                c_env->variables);
...
c_register_tags(cl_env_ptr env, cl_object all_tags)
{
        const cl_compiler_ptr c_env = env->c_env;
	cl_object loc = new_location(c_env);
	c_env->variables = CONS(cl_list(4, ECL_SYM(":TAG",1315), all_tags, ECL_NIL, loc),
                                c_env->variables);
	return ecl_fixnum(ECL_CONS_CDR(loc));
}
...
{
        const cl_compiler_ptr c_env = env->c_env;
	c_env->variables = CONS(cl_list(4, ECL_SYM(":FUNCTION",1241), name, ECL_NIL,
                                        new_location(c_env)),
                                c_env->variables);
	c_env->macros = CONS(cl_list(2, name, ECL_SYM("FUNCTION",396)), c_env->macros);
}
...
{
        const cl_compiler_ptr c_env = env->c_env;
	c_env->variables = CONS(cl_list(3, name, ECL_SYM("SI::SYMBOL-MACRO",1159), exp_fun),
                                c_env->variables);
}
...
{
        const cl_compiler_ptr c_env = env->c_env;
	c_env->macros = CONS(cl_list(3, name, ECL_SYM("SI::MACRO",1101), exp_fun), c_env->macros);
}
*/
...
c_register_var(cl_env_ptr env, cl_object var, bool special, bool bound)
{
	...
	c_env->variables = CONS(cl_list(4, var,
					special? ECL_SYM("SPECIAL",789) : ECL_NIL,
					bound? ECL_T : ECL_NIL,
					new_location(c_env)),
				c_env->variables);
}
          ...
          cl_compiler_env_ptr old)
{
	...
	if (old) {
		*new = *old;
		...
		new->code_walker = ECL_SYM_VAL(the_env, ECL_SYM("SI::*CODE-WALKER*",1588));
		...
		for (env = new->variables; !Null(env); env = CDR(env)) {
			...
			if (ECL_ATOM(record))
				continue;
			if (ECL_SYMBOLP(CAR(record)) && CADR(record) != ECL_SYM("SI::SYMBOL-MACRO",1159)) {
				continue;
			} else {
				new->lexical_level = 1;
				break;
...
c_tag_ref(cl_env_ptr env, cl_object the_tag, cl_object the_type)
{
	...
	for (l = c_env->variables; CONSP(l); l = ECL_CONS_CDR(l)) {
		...
		if (type == ECL_SYM(":TAG",1315)) {
			...
			n++;
		} else if (type == ECL_SYM(":BLOCK",1202) || type == ECL_SYM(":FUNCTION",1241)) {
...
c_var_ref(cl_env_ptr env, cl_object var, int allow_symbol_macro, bool ensure_defined)
{
	...
	for (l = c_env->variables; CONSP(l); l = ECL_CONS_CDR(l)) {
		...
		if (name == ECL_SYM(":BLOCK",1202) || name == ECL_SYM(":TAG",1315) || name == ECL_SYM(":FUNCTION",1241)) {
			n++;
		} else if (name == ECL_SYM(":DECLARE",986)) {
			/* Ignored */
		} else if (name != var) {
			/* Symbol not yet found. Only count locals. */
			if (Null(special)) n++;
		} else if (special == ECL_SYM("SI::SYMBOL-MACRO",1159)) {
	...
	if (ensure_defined) {
		l = ecl_symbol_value(ECL_SYM("EXT::*ACTION-ON-UNDEFINED-VARIABLE*",1596));
		if (l != ECL_NIL) {
...
c_undo_bindings(cl_env_ptr the_env, cl_object old_vars, int only_specials)
{
	...
	for (env = c_env->variables; env != old_vars && !Null(env); env = ECL_CONS_CDR(env))
	{
		...
		name = ECL_CONS_CAR(record);
                record = ECL_CONS_CDR(record);
		special = ECL_CONS_CAR(record);
		if (name == ECL_SYM(":BLOCK",1202) || name == ECL_SYM(":TAG",1315)) {
			(void)0;
		} else if (name == ECL_SYM(":FUNCTION",1241) || Null(special)) {
...
c_call(cl_env_ptr env, cl_object args, int flags) {
	...
	if (env->c_env->stepping) {
		...
		flags = FLAG_VALUES;
		   ...
		   ((flags & FLAG_GLOBAL) || Null(c_tag_ref(env, name, ECL_SYM(":FUNCTION",1241)))))
	{
...
perform_c_case(cl_env_ptr env, cl_object args, int flags) {
	...
	if (ECL_SYM("OTHERWISE",615) == test || test == ECL_T) {
...
eval_when_flags(cl_object situation)
{
        ...
        for (p = situation; p != ECL_NIL; p = ECL_CONS_CDR(p)) {
                ...
                if (keyword == ECL_SYM("LOAD",487))
                        code |= FLAG_LOAD;
                else if (keyword == ECL_SYM(":LOAD-TOPLEVEL",1270))
                        code |= FLAG_LOAD;
                else if (keyword == ECL_SYM("COMPILE",233))
                        code |= FLAG_COMPILE;
                else if (keyword == ECL_SYM(":COMPILE-TOPLEVEL",1207))
                        code |= FLAG_COMPILE;
                else if (keyword == ECL_SYM("EVAL",339))
                        code |= FLAG_EXECUTE;
                else if (keyword == ECL_SYM(":EXECUTE",1231))
                        code |= FLAG_EXECUTE;
        }
        return code;
}
...
asm_function(cl_env_ptr env, cl_object function, int flags) {
	if (!Null(si_valid_function_name_p(function))) {
		cl_object ndx = c_tag_ref(env, function, ECL_SYM(":FUNCTION",1241));
		if (Null(ndx)) {
		...
		cl_object name;
                if (kind == ECL_SYM("LAMBDA",452)) {
			name = ECL_NIL;
                } else if (kind == ECL_SYM("EXT::LAMBDA-BLOCK",1339)) {
...
c_go(cl_env_ptr env, cl_object args, int flags) {
	cl_object tag = pop(&args);
	cl_object info = c_tag_ref(env, tag, ECL_SYM(":TAG",1315));
	if (Null(info))
...
c_macrolet(cl_env_ptr the_env, cl_object args, int flags)
{
	...
	cl_object env = funcall(3, ECL_SYM("SI::CMP-ENV-REGISTER-MACROLET",1603), pop(&args),
...
c_multiple_value_setq(cl_env_ptr env, cl_object orig_args, int flags) {
	...
	for (orig_vars = pop(&args); !Null(orig_vars); ) {
		...
		if (!ECL_SYMBOLP(v)) {
			 ...
			 */
			args = orig_args;
			return compile_form(env, cl_listX(3, ECL_SYM("SETF",750),
                                                          CONS(ECL_SYM("VALUES",895), CAR(args)),
                                                          CDR(args)),
					    flags);
		}
		vars = CONS(v, vars);
	...
	if (nvars == 0) {
		/* No variables */
		return compile_form(env, cl_list(2, ECL_SYM("VALUES",895), values), flags);
	}
...
c_psetq(cl_env_ptr env, cl_object old_args, int flags) {
	...
	*/
	...
	if (use_psetf) {
		return compile_form(env, CONS(ECL_SYM("PSETF",675), args), flags);
	}
	do {
...
{
	cl_object ndx = c_tag_ref(env, name, ECL_SYM(":BLOCK",1202));
...
c_setq(cl_env_ptr env, cl_object args, int flags) {
	...
	do {
		...
		if (ECL_SYMBOLP(var)) {
			...
			compile_form(env, cl_list(3, ECL_SYM("SETF",750), var, value), flags);
		}
	} while (!Null(args));
	return flags;
}
...
declared special and appear in a symbol-macrolet.", 1, name);
		...
		definition = cl_list(2, arglist, cl_list(2, ECL_SYM("QUOTE",679), expansion));
...
need_to_make_load_form_p(cl_object o)
{
        ...
        case t_list:
                if (Null(o)) return 0;
        default:
                return _ecl_funcall3(ECL_SYM("CLOS::NEED-TO-MAKE-LOAD-FORM-P",1577), o, ECL_NIL)
                        != ECL_NIL;
        }
}
...
maybe_make_load_forms(cl_env_ptr env, cl_object constant)
{
        ...
        if (!need_to_make_load_form_p(constant))
                return;
        make = _ecl_funcall2(ECL_SYM("MAKE-LOAD-FORM",951), constant);
...
compile_form(cl_env_ptr env, cl_object stmt, int flags) {
	...
	ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1612), stmt);
 BEGIN:
	...
	if (ECL_SYMBOLP(function)) {
		...
		if (index != OBJNULL) {
			...
			if (c_env->stepping && function != ECL_SYM("FUNCTION",396) &&
			    c_env->lexical_level)
...
si_process_lambda(cl_object lambda)
{
	...
	specials = env->values[3];
        ...
        lambda_list = si_process_lambda_list(lambda_list, ECL_SYM("FUNCTION",396));
        {
...
#define assert_var_name(v) \
	if (context == ECL_SYM("FUNCTION",396)) { \
...
LOOP:
        if (Null(lambda_list))
                goto OUTPUT;
	if (!ECL_LISTP(lambda_list)) {
		unlikely_if (context == ECL_SYM("FUNCTION",396) || context == ECL_SYM("FTYPE",393))
			goto ILLEGAL_LAMBDA;
                v = lambda_list;
                lambda_list = ECL_NIL;
                goto REST;
	...
	if (v == ECL_SYM("&OPTIONAL",11)) {
		unlikely_if (stage >= AT_OPTIONALS)
			goto ILLEGAL_LAMBDA;
		stage = AT_OPTIONALS;
		goto LOOP;
	}
	if (v == ECL_SYM("&REST",12) || (v == ECL_SYM("&BODY",8) && (context == ECL_SYM("SI::MACRO",1101) || context == ECL_SYM("DESTRUCTURING-BIND",300)))) {
...
REST:		unlikely_if (stage >= AT_REST)
	...
	if (v == ECL_SYM("&ALLOW-OTHER-KEYS",6)) {
	...
	case AT_OPTIONALS: {
		...
		if (!ECL_ATOM(v) && (context != ECL_SYM("FTYPE",393))) {
			cl_object x = v;
...
ecl_make_lambda(cl_env_ptr env, cl_object name, cl_object lambda) {
	...
	ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1612),
		     cl_listX(3, ECL_SYM("EXT::LAMBDA-BLOCK",1339), name, lambda));
	...
	c_register_var(env, ECL_SYM("SI::FUNCTION-BOUNDARY",1833), TRUE, FALSE);
	...
	if (!Null(name)) {
		compile_form(env, cl_listX(3, ECL_SYM("BLOCK",137), si_function_block_name(name),
                                         body), FLAG_VALUES);
	} else {
	...
	asm_op(env, OP_EXIT);
        ...
        if (Null(ecl_symbol_value(ECL_SYM("SI::*KEEP-DEFINITIONS*",1024))))
                lambda = ECL_NIL;
...
ecl_function_block_name(cl_object name)
{
	if (ECL_SYMBOLP(name)) {
		return name;
	} else if (CONSP(name) && ECL_CONS_CAR(name) == ECL_SYM("SETF",750)) {
...
init_compiler()
{
	cl_object dispatch_table =
		...
		cl__make_hash_table(ECL_SYM("EQ",333), ecl_make_fixnum(128), /* size */
                                    cl_core.rehash_size,
