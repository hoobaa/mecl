#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/serialize.d"
...
alloc(pool_t pool, cl_index size)
{
        ...
        if (next_fillp >= pool->data->vector.dim) {
                ...
                pool->data = _ecl_funcall3(ECL_SYM("ADJUST-ARRAY",83), pool->data,
...
{
        pool->data = si_make_vector(ECL_SYM("EXT::BYTE8",1330),
                                    ecl_make_fixnum(1024),
                                    ECL_T,
                                    ecl_make_fixnum(2 * sizeof(cl_index)),
                                    ECL_NIL,
                                    ecl_make_fixnum(0));
        pool->hash = cl__make_hash_table(ECL_SYM("EQL",334), ecl_make_fixnum(256),
                                         cl_core.rehash_size,
