#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/interpreter.d"
...
too_many_arguments(register cl_object bytecodes, register cl_object frame)
{
        FEprogram_error("Too many arguments passed to "
                        ...
                        2, bytecodes, cl_apply(2, ECL_SYM("LIST",481), frame));
}
...
unknown_keyword(register cl_object bytecodes, register cl_object frame)
{
        FEprogram_error("Unknown keyword argument passed to function ~S.~&"
                        ...
                        cl_apply(2, ECL_SYM("LIST",481), frame));
}
...
ecl_interpret(cl_object frame, cl_object env, cl_object bytecodes)
{
 ...
 BEGIN:
	...
	*/
	...
	CASE(OP_CAR); {
		if (ecl_unlikely(!LISTP(reg0)))
                        FEwrong_type_only_arg(ecl_make_fixnum(/*CAR*/180), reg0, ecl_make_fixnum(/*CONS*/251));
		reg0 = CAR(reg0);
		THREAD_NEXT;
	}
	...
	CASE(OP_CDR); {
		if (ecl_unlikely(!LISTP(reg0)))
                        FEwrong_type_only_arg(ecl_make_fixnum(/*CDR*/198), reg0, ecl_make_fixnum(/*CONS*/251));
		reg0 = CDR(reg0);
		THREAD_NEXT;
	}
	...
	CASE(OP_PUSHKEYS); {
                ...
                if (count) {
                        if (Null(aok)) {
                                ...
                                cl_object *p = first;
                                for (; p != last; ++p) {
                                        if (*(p++) == ECL_SYM(":ALLOW-OTHER-KEYS",1197)) {
                                                if (!Null(*p)) aok |= mask;
                                                mask <<= 1;
                                                count -= 2;
                                        }
	...
	*/
	...
	CASE(OP_ENDP);
		if (ecl_unlikely(!LISTP(reg0)))
                        FEwrong_type_only_arg(ecl_make_fixnum(/*ENDP*/330), reg0, ecl_make_fixnum(/*LIST*/481));
	CASE(OP_NOT); {
	...
	*/
	...
	CASE(OP_STEPIN); {
		cl_object form;
		cl_object a = ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-ACTION*",1033));
		cl_index n;
		...
		if (a == ECL_T) {
			 ...
			 * what to do. */
			ECL_SETQ(the_env, ECL_SYM("SI::*STEP-LEVEL*",1032),
				 cl_1P(ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-LEVEL*",1032))));
			ECL_STACK_PUSH(the_env, form);
			INTERPRET_FUNCALL(form, the_env, frame_aux, 1, ECL_SYM("SI::STEPPER",1148));
		} else if (a != ECL_NIL) {
			...
			ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1033), cl_1P(a));
		} else {
	...
	CASE(OP_STEPCALL); {
		...
		GET_OPARG(n, vector);
		SETUP_ENV(the_env);
		if (ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-ACTION*",1033)) == ECL_T) {
			ECL_STACK_PUSH(the_env, reg0);
			INTERPRET_FUNCALL(reg0, the_env, frame_aux, 1, ECL_SYM("SI::STEPPER",1148));
		}
	...
	CASE(OP_STEPOUT); {
		...
		if (a == ECL_T) {
			...
			ECL_SETQ(the_env, ECL_SYM("SI::*STEP-LEVEL*",1032),
				 cl_1M(ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-LEVEL*",1032))));
		} else if (a == ecl_make_fixnum(0)) {
			/* We are back to the level in which the user
			 * selected to step over. */
			ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1033), ECL_T);
		} else if (a != ECL_NIL) {
			ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1033), cl_1M(a));
		} else {
