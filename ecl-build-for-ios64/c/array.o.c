#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/array.d"
...
static const cl_object ecl_aet_name[] = {
        ...
        ECL_SYM("SINGLE-FLOAT",778),      /* ecl_aet_sf */
        ECL_SYM("DOUBLE-FLOAT",315),      /* ecl_aet_df */
        ECL_SYM("BIT",123),               /* ecl_aet_bit: cannot be handled with this code */
        ECL_SYM("EXT::CL-FIXNUM",1582),    /* ecl_aet_fix */
        ECL_SYM("EXT::CL-INDEX",1583),     /* ecl_aet_index */
        ECL_SYM("EXT::BYTE8",1330),        /* ecl_aet_b8 */
        ECL_SYM("EXT::INTEGER8",1335),     /* ecl_aet_i8 */
#ifdef ecl_uint16_t
        ECL_SYM("EXT::BYTE16",1331),
        ECL_SYM("EXT::INTEGER16",1336),
#endif
#ifdef ecl_uint32_t
        ECL_SYM("EXT::BYTE32",1332),
        ECL_SYM("EXT::INTEGER32",1337),
#endif
#ifdef ecl_uint64_t
        ECL_SYM("EXT::BYTE64",1333),
        ECL_SYM("EXT::INTEGER64",1338),
#endif
#ifdef ECL_UNICODE
        ECL_SYM("CHARACTER",222),         /* ecl_aet_ch */
#endif
        ECL_SYM("BASE-CHAR",120)          /* ecl_aet_bc */
};
...
{
	cl_object type = cl_list(3, ECL_SYM("INTEGER",437), ecl_make_fixnum(0),
...
FEwrong_dimensions(cl_object a, cl_index rank)
{
        cl_object list = cl_make_list(3, ecl_make_fixnum(rank),
                                      ECL_SYM(":INITIAL-ELEMENT",1251), ECL_SYM("*",18));
        cl_object type = cl_list(3, ECL_SYM("ARRAY",96), ECL_SYM("*",18), list);
...
ecl_to_index(cl_object n)
{
	...
	default:
		FEwrong_type_only_arg(ecl_make_fixnum(/*COERCE*/231), n, ecl_make_fixnum(/*FIXNUM*/372));
	}
}
...
#line 123
{
	...
	case t_array:
		...
		for (i = j = 0;  i < r;  i++) {
			cl_index s = checked_index(ecl_make_fixnum(/*AREF*/92), x, i,
                                                   ecl_va_arg(indx),
                                                   x->array.dims[i]);
	...
	case t_bitvector:
		if (r != 1)
			FEerror("Wrong number of indices.", 0);
		j = checked_index(ecl_make_fixnum(/*AREF*/92), x, -1, ecl_va_arg(indx), x->vector.dim);
		break;
	default:
                FEwrong_type_nth_arg(ecl_make_fixnum(/*AREF*/92), 1, x, ecl_make_fixnum(/*ARRAY*/96));
	}
	{
#line 150
		#line 150
...
ecl_aref(cl_object x, cl_index index)
{
        ...
        if (ecl_unlikely(index >= x->array.dim)) {
                FEwrong_index(ecl_make_fixnum(/*ROW-MAJOR-AREF*/731), x, -1, ecl_make_fixnum(index),
                              x->array.dim);
        }
...
ecl_aref1(cl_object x, cl_index index)
{
        ...
        if (ecl_unlikely(index >= x->array.dim)) {
                FEwrong_index(ecl_make_fixnum(/*AREF*/92), x, -1, ecl_make_fixnum(index),
                              x->array.dim);
        }
        return ecl_aref_unsafe(x, index);
}
...
#line 265
{
	...
	case t_array:
		if (ecl_unlikely(r != x->array.rank))
			FEerror("Wrong number of indices.", 0);
		for (i = j = 0;  i < r;  i++) {
			cl_index s = checked_index(ecl_make_fixnum(/*SI::ASET*/1041), x, i,
                                                   ecl_va_arg(dims),
                                                   x->array.dims[i]);
			j = j*(x->array.dims[i]) + s;
		}
	...
	case t_bitvector:
		if (ecl_unlikely(r != 1))
			FEerror("Wrong number of indices.", 0);
		j = checked_index(ecl_make_fixnum(/*SI::ASET*/1041), x, -1, ecl_va_arg(dims),
                                  x->vector.dim);
		break;
	default:
                FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::ASET*/1041), 1, x, ecl_make_fixnum(/*ARRAY*/96));
	}
	v = ecl_va_arg(dims);
	{
#line 295
		   ...
		   cl_object fillp, cl_object displ, cl_object disploff)
{
	...
	if (ECL_FIXNUMP(dims)) {
		return si_make_vector(etype, dims, adj, fillp, displ, disploff);
                ...
                FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/522), 1, dims,
                                     cl_list(3, ECL_SYM("OR",614), ECL_SYM("LIST",481), ECL_SYM("FIXNUM",372)));
        }
	r = ecl_length(dims);
	...
	for (i = 0, s = 1;  i < r;  i++, dims = ECL_CONS_CDR(dims)) {
                                 ...
                                 ecl_fixnum_greater(d, ecl_make_fixnum(ECL_ARRAY_DIMENSION_LIMIT))))
                {
                        ...
                        FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/522), 1, d, type);
                }
                j = ecl_fixnum(d);
		s *= (x->array.dims[i] = j);
		if (ecl_unlikely(s > ECL_ARRAY_TOTAL_LIMIT)) {
                        ...
                        FEwrong_type_key_arg(ecl_make_fixnum(/*MAKE-ARRAY*/522), ecl_make_fixnum(/*ARRAY-TOTAL-SIZE*/107),
 ...
 AGAIN:
	aet = ecl_symbol_to_elttype(etype);
        if (ecl_unlikely(!ECL_FIXNUMP(dim) || ecl_fixnum_minusp(dim) ||
                ...
                cl_object type = ecl_make_integer_type(ecl_make_fixnum(0),
                                                       ecl_make_fixnum(ECL_ARRAY_DIMENSION_LIMIT));
                FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/522), 1, dim, type);
        }
        d = ecl_fixnum(dim);
	if (aet == ecl_aet_bc) {
	...
	if (Null(fillp)) {
		...
		fillp = ecl_type_error(ECL_SYM("MAKE-ARRAY",522),"fill pointer",fillp,
				       cl_list(3,ECL_SYM("OR",614),cl_list(3,ECL_SYM("MEMBER",552),ECL_NIL,ECL_T),
					       cl_list(3,ECL_SYM("INTEGER",437),ecl_make_fixnum(0),
						       dim)));
		goto AGAIN;
	}
	x->vector.fillp = f;
...
{
 BEGIN:
	if (x == ECL_SYM("BASE-CHAR",120))
		return(ecl_aet_bc);
#ifdef ECL_UNICODE
	if (x == ECL_SYM("CHARACTER",222))
		return(ecl_aet_ch);
#endif
	else if (x == ECL_SYM("BIT",123))
		return(ecl_aet_bit);
	else if (x == ECL_SYM("EXT::CL-FIXNUM",1582))
		return(ecl_aet_fix);
	else if (x == ECL_SYM("EXT::CL-INDEX",1583))
		return(ecl_aet_index);
	else if (x == ECL_SYM("SINGLE-FLOAT",778) || x == ECL_SYM("SHORT-FLOAT",756))
		return(ecl_aet_sf);
	else if (x == ECL_SYM("DOUBLE-FLOAT",315))
		return(ecl_aet_df);
	else if (x == ECL_SYM("LONG-FLOAT",508)) {
#ifdef ECL_LONG_FLOAT
		return(ecl_aet_object);
#else
		return(ecl_aet_df);
#endif
	} else if (x == ECL_SYM("EXT::BYTE8",1330))
		return(ecl_aet_b8);
	else if (x == ECL_SYM("EXT::INTEGER8",1335))
		return(ecl_aet_i8);
#ifdef ecl_uint16_t
	else if (x == ECL_SYM("EXT::BYTE16",1331))
		return(ecl_aet_b16);
	else if (x == ECL_SYM("EXT::INTEGER16",1336))
		return(ecl_aet_i16);
#endif
#ifdef ecl_uint32_t
	else if (x == ECL_SYM("EXT::BYTE32",1332))
		return(ecl_aet_b32);
	else if (x == ECL_SYM("EXT::INTEGER32",1337))
		return(ecl_aet_i32);
#endif
#ifdef ecl_uint64_t
	else if (x == ECL_SYM("EXT::BYTE64",1333))
		return(ecl_aet_b64);
	else if (x == ECL_SYM("EXT::INTEGER64",1338))
		return(ecl_aet_i64);
#endif
...
ecl_displace(cl_object from, cl_object to, cl_object offset)
{
	...
	fromtype = from->array.elttype;
        if (ecl_unlikely(!ECL_FIXNUMP(offset) || ((j = ecl_fixnum(offset)) < 0))) {
                FEwrong_type_key_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/83), ecl_make_fixnum(/*:DISPLACED-INDEX-OFFSET*/1220),
                                     offset, ecl_make_fixnum(/*FIXNUM*/372));
        }
...
si_array_raw_data(cl_object x)
{
		...
		if (ECL_ARRAY_HAS_FILL_POINTER_P(x)) {
			fillp = ecl_make_fixnum(x->vector.fillp * ecl_aet_size[et]);
                ...
                output = si_make_vector(ECL_SYM("EXT::BYTE8",1330),
...
{
        if (ecl_unlikely(!ECL_ARRAYP(x)))
                FEwrong_type_argument(ecl_make_fixnum(/*ARRAY*/96), x);
...
ecl_array_rank(cl_object a)
{
	...
	case t_bitvector:
                return 1;
	default:
                FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-DIMENSION*/97), a, ecl_make_fixnum(/*ARRAY*/96));
	}
}
...
cl_array_displacement(cl_object a)
{
	...
	cl_index offset;
        ...
        if (ecl_unlikely(!ECL_ARRAYP(a)))
                FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-DISPLACEMENT*/100), a, ecl_make_fixnum(/*ARRAY*/96));
...
cl_svref(cl_object x, cl_object index)
{
	...
	{
                FEwrong_type_nth_arg(ecl_make_fixnum(/*SVREF*/838),1,x,ecl_make_fixnum(/*SIMPLE-VECTOR*/774));
	}
        i = checked_index(ecl_make_fixnum(/*SVREF*/838), x, -1, index, x->vector.dim);
...
si_svset(cl_object x, cl_object index, cl_object v)
{
	...
	if (ecl_unlikely(ecl_t_of(x) != t_vector ||
                         ...
                         (cl_elttype)x->vector.elttype != ecl_aet_object))
	{
		FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::SVSET*/1158),1,x,ecl_make_fixnum(/*SIMPLE-VECTOR*/774));
	}
        i = checked_index(ecl_make_fixnum(/*SVREF*/838), x, -1, index, x->vector.dim);
...
cl_array_has_fill_pointer_p(cl_object a)
{
	...
	case t_base_string:
		...
		break;
	default:
                FEwrong_type_nth_arg(ecl_make_fixnum(/*ARRAY-HAS-FILL-POINTER-P*/102),1,a,ecl_make_fixnum(/*ARRAY*/96));
	}
...
cl_fill_pointer(cl_object a)
{
	const cl_env_ptr the_env = ecl_process_env();
        if (ecl_unlikely(!ECL_VECTORP(a)))
                FEwrong_type_only_arg(ecl_make_fixnum(/*FILL-POINTER*/362), a, ecl_make_fixnum(/*VECTOR*/898));
	if (ecl_unlikely(!ECL_ARRAY_HAS_FILL_POINTER_P(a))) {
                const char *type = "(AND VECTOR (SATISFIES ARRAY-HAS-FILL-POINTER-P))";
		FEwrong_type_nth_arg(ecl_make_fixnum(/*FILL-POINTER*/362), 1, a, ecl_read_from_cstring(type));
	}
...
si_fill_pointer_set(cl_object a, cl_object fp)
{
	const cl_env_ptr the_env = ecl_process_env();
        ...
        if (ecl_unlikely(!ECL_VECTORP(a) || !ECL_ARRAY_HAS_FILL_POINTER_P(a))) {
                const char *type = "(AND VECTOR (SATISFIES ARRAY-HAS-FILL-POINTER-P))";
		FEwrong_type_nth_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/83), 1, a,
        ...
        if (ecl_unlikely(!ECL_FIXNUMP(fp) || ((i = ecl_fixnum(fp)) < 0) ||
                ...
                FEwrong_type_key_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/83), ecl_make_fixnum(/*:FILL-POINTER*/1238), fp, type);
        }
        a->vector.fillp = i;
