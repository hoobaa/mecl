#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/gfun.d"
...
clos_set_funcallable_instance_function(cl_object x, cl_object function_or_t)
{
	if (ecl_unlikely(!ECL_INSTANCEP(x)))
                FEwrong_type_nth_arg(ecl_make_fixnum(/*CLOS::SET-FUNCALLABLE-INSTANCE-FUNCTION*/1540),
                                     1, x, ecl_make_fixnum(/*EXT::INSTANCE*/1342));
	...
	if (function_or_t == ECL_T) {
	...
	} else if (function_or_t == ECL_SYM("STANDARD-GENERIC-FUNCTION",974)) {
		x->instance.isgf = ECL_RESTRICTED_DISPATCH;
                x->instance.entry = generic_function_dispatch_vararg;
	} else if (function_or_t == ECL_NIL) {
		x->instance.isgf = ECL_NOT_FUNCALLABLE;
                x->instance.entry = FEnot_funcallable_vararg;
	} else if (function_or_t == ECL_SYM("CLOS::STANDARD-OPTIMIZED-READER-METHOD",1566)) {
	...
	} else if (function_or_t == ECL_SYM("CLOS::STANDARD-OPTIMIZED-WRITER-METHOD",1567)) {
		/* WARNING: We assume that f(a,...) behaves as f(a,b) */
		...
		FEwrong_type_argument(ECL_SYM("FUNCTION",396), function_or_t);
	} else {
		reshape_instance(x, +1);
...
generic_compute_applicable_method(cl_env_ptr env, cl_object frame, cl_object gf)
{
	/* method not cached */
	cl_object memoize;
	cl_object methods = _ecl_funcall3(ECL_SYM("CLOS::COMPUTE-APPLICABLE-METHODS-USING-CLASSES",1495),
					  gf, frame_to_classes(frame));
	unlikely_if (Null(memoize = env->values[1])) {
		cl_object arglist = frame_to_list(frame);
		methods = _ecl_funcall3(ECL_SYM("COMPUTE-APPLICABLE-METHODS",936),
					gf, arglist);
		unlikely_if (methods == ECL_NIL) {
			cl_object func = _ecl_funcall3(ECL_SYM("NO-APPLICABLE-METHOD",959),
						       gf, arglist);
...
restricted_compute_applicable_method(cl_env_ptr env, cl_object frame, cl_object gf)
{
	...
	unlikely_if (methods == ECL_NIL) {
		cl_object func = _ecl_funcall3(ECL_SYM("NO-APPLICABLE-METHOD",959), gf, arglist);
		frame->frame.base[0] = OBJNULL;
		env->values[1] = ECL_NIL;
		return func;
	}
