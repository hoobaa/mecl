...
assert_type_process(cl_object o)
{
	if (ecl_t_of(o) != t_process)
		FEwrong_type_argument(ecl_make_fixnum(/*MP::PROCESS*/1395), o);
}
...
thread_cleanup(void *aux)
{
	...
	mp_barrier_unblock(3, process->process.exit_barrier, ECL_SYM(":DISABLE",1458), ECL_T);
...
#endif
{
	...
	*/
	ECL_CATCH_ALL_BEGIN(env) {
		...
		ecl_bds_bind(env, ECL_SYM("MP::*CURRENT-PROCESS*",1398), process);
...
{
        cl_env_ptr env = ecl_process_env();
        ECL_CATCH_BEGIN(env,ECL_SYM("MP::SUSPEND-LOOP",1432)) {
                for ( ; ; ) {
...
{
        cl_env_ptr the_env = ecl_process_env();
        if (frs_sch(ECL_SYM("MP::SUSPEND-LOOP",1432))) {
                cl_throw(ECL_SYM("MP::SUSPEND-LOOP",1432));
        }
	ecl_return0(the_env);
}
...
{
        return mp_interrupt_process(process, ECL_SYM("MP::SUSPEND-LOOP",1432));
}

cl_object
mp_process_resume(cl_object process)
{
        return mp_interrupt_process(process, ECL_SYM("MP::BREAK-SUSPEND-LOOP",1433));
}

cl_object
mp_process_kill(cl_object process)
{
	return mp_interrupt_process(process, ECL_SYM("MP::EXIT-PROCESS",1400));
}
