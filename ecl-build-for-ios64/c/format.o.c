#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/format.d"
...
{
	cl_error(7, ECL_SYM("SI::FORMAT-ERROR",1074),
		 ECL_SYM(":FORMAT-CONTROL",1240), make_constant_base_string(s),
		 ECL_SYM(":CONTROL-STRING",1211), fmt->ctl_str,
		 ECL_SYM(":OFFSET",1279), ecl_make_fixnum(fmt->ctl_index));
}
	    ...
	    int radix, int mincol, ecl_character padchar, ecl_character commachar)
{
	...
	if (!ECL_FIXNUMP(x) && ecl_t_of(x) != t_bignum) {
		fmt_prepare_aux_stream(fmt);
		ecl_bds_bind(env, ECL_SYM("*PRINT-ESCAPE*",48), ECL_NIL);
		ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",45), ecl_make_fixnum(radix));
	...
	fmt_prepare_aux_stream(fmt);
	ecl_bds_bind(env, ECL_SYM("*PRINT-RADIX*",56), ECL_NIL);
	ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",45), ecl_make_fixnum(radix));
	si_write_object(x, fmt->aux_stream);
	ecl_bds_unwind_n(env, 2);
...
fmt_radix(format_stack fmt, bool colon, bool atsign)
{
	...
	if (fmt->nparam == 0) {
		...
		fmt_prepare_aux_stream(fmt);
		ecl_bds_bind(env, ECL_SYM("*PRINT-RADIX*",56), ECL_NIL);
		ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",45), ecl_make_fixnum(10));
		si_write_object(x, fmt->aux_stream);
		ecl_bds_unwind_n(env, 2);
		s = 0;
...
fmt_character(format_stack fmt, bool colon, bool atsign)
{
	...
	ensure_param(fmt, 0);
	x = fmt_advance(fmt);
	x = ecl_check_cl_type(ECL_SYM("FORMAT",387),x,t_character);
	if (!colon && !atsign) {
...
fmt_exponential_float(format_stack fmt, bool colon, bool atsign)
{
	...
	if (sign < 0)
		...
		ecl_write_char('+', fmt->stream);
	writestr_stream(b, fmt->stream);
	y = ecl_symbol_value(ECL_SYM("*READ-DEFAULT-FLOAT-FORMAT*",62));
	if (exponentchar < 0) {
		if (y == ECL_SYM("LONG-FLOAT",508)) {
#ifdef ECL_LONG_FLOAT
			t = t_longfloat;
#else
			t = t_doublefloat;
#endif
		} else if (y == ECL_SYM("DOUBLE-FLOAT",315)) {
			t = t_doublefloat;
		} else if (y == ECL_SYM("SINGLE-FLOAT",778)) {
			t = t_singlefloat;
		} else {
			t = t_singlefloat;
		}
...
#ifdef ECL_UNICODE
		string = ecl_type_error(ECL_SYM("FORMAT",387), "argument", string, ECL_SYM("STRING",805));
#else
		string = ecl_type_error(ECL_SYM("FORMAT",387), "argument", string, ECL_SYM("BASE-STRING",121));
#endif
	fmt.stream = strm;
	fmt_set_arg_list(&fmt, output);
	fmt.jmp_buf = &fmt_jmp_buf0;
	if (ecl_symbol_value(ECL_SYM("SI::*INDENT-FORMATTED-OUTPUT*",1021)) != ECL_NIL)
...
// ------------------------------5
	if (Null(strm)) {
		...
		strm = ecl_symbol_value(ECL_SYM("*STANDARD-OUTPUT*",67));
	}
	if (ecl_stringp(strm)) {
		output = strm;
		if (!ECL_ARRAY_HAS_FILL_POINTER_P(output)) {
			cl_error(7, ECL_SYM("SI::FORMAT-ERROR",1074),
				 ECL_SYM(":FORMAT-CONTROL",1240),
				 make_constant_base_string(
"Cannot output to a non adjustable string."),
				 ECL_SYM(":CONTROL-STRING",1211), string,
				 ECL_SYM(":OFFSET",1279), ecl_make_fixnum(0));
                }
	...
	if (!Null(cl_functionp(string))) {
		...
		_ecl_funcall4(ECL_SYM("SI::FORMATTER-AUX",1075), strm, string,
