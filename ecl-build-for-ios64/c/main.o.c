...
maybe_fix_console_stream(cl_object stream)
{
	...
	external_format = cl_list(2, ecl_make_keyword(known_cp[i].name), ECL_SYM(":CRLF",1690));
        nlogd(">>>");
...
cl_boot(int argc, char **argv)
{
	...
	ECL_SET(ECL_SYM("FFI::C-INT-MAX",1956), ecl_make_integer(INT_MAX));
	ECL_SET(ECL_SYM("FFI::C-INT-MIN",1957), ecl_make_integer(INT_MIN));
	ECL_SET(ECL_SYM("FFI::C-LONG-MAX",1960), ecl_make_integer(LONG_MAX));
	ECL_SET(ECL_SYM("FFI::C-LONG-MIN",1961), ecl_make_integer(LONG_MIN));
	ECL_SET(ECL_SYM("FFI::C-UINT-MAX",1964), ecl_make_unsigned_integer(UINT_MAX));
	ECL_SET(ECL_SYM("FFI::C-ULONG-MAX",1966), ecl_make_unsigned_integer(ULONG_MAX));
#ifdef ecl_long_long_t
	ECL_SET(ECL_SYM("FFI::C-LONG-LONG-MAX",1962), ecl_make_unsigned_integer(LLONG_MAX));
	ECL_SET(ECL_SYM("FFI::C-ULONG-LONG-MAX",1967), ecl_make_unsigned_integer(ULLONG_MAX));
#endif
	 ...
	 */
#ifdef ECL_THREADS
	ECL_SET(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1426),
		ecl_make_lock(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1426), 1));
#endif
	aux = cl_list(
#ifdef ENABLE_DLOPEN
		11,
                CONS(str_fas, ECL_SYM("SI::LOAD-BINARY",1587)),
		CONS(str_fasl, ECL_SYM("SI::LOAD-BINARY",1587)),
		CONS(str_fasb, ECL_SYM("SI::LOAD-BINARY",1587)),
		CONS(str_FASB, ECL_SYM("SI::LOAD-BINARY",1587)),
#else
		7,
#endif
		...
		CONS(str_LSP, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_LISP, ECL_SYM("SI::LOAD-SOURCE",1099)),
		CONS(str_fasc, ECL_SYM("SI::LOAD-BYTECODES",1836)),
		CONS(str_FASC, ECL_SYM("SI::LOAD-BYTECODES",1836)),
	...
	if (cl_fboundp(ECL_SYM("EXT::MAKE-ENCODING",1696)) != ECL_NIL) {
...
#endif
        ECL_SET(ECL_SYM("EXT::*PROGRAM-EXIT-CODE*",1739), code);
