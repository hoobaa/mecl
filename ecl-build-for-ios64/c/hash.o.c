...
#line 743
{
	...
	if (!Null(weakness)) {
		...
		} else if (weakness == ECL_SYM(":VALUE",1945)) {
			hash->hash.weak = ecl_htt_weak_value;
		} else if (weakness == ECL_SYM(":KEY-AND-VALUE",1946)) {
			hash->hash.weak = ecl_htt_weak_key_and_value;
		} else {
			FEwrong_type_key_arg(ecl_make_fixnum(/*MAKE-HASH-TABLE*/528),
					     ecl_make_fixnum(/*:WEAKNESS*/1947),
					     cl_list(5, ECL_SYM("MEMBER",552),
						     ECL_NIL, ECL_SYM(":KEY",1262), ECL_SYM(":VALUE",1945),
						     ECL_SYM(":KEY-AND-VALUE",1946)),
					     weakness);
		}
...
si_hash_table_weakness(cl_object ht)
{
	...
	case ecl_htt_weak_value: output = ECL_SYM(":VALUE",1945); break;
	case ecl_htt_weak_key_and_value: output = ECL_SYM(":KEY-AND-VALUE",1946); break;
...
si_hash_table_content(cl_object ht)
{
	cl_index i;
	cl_object output = ECL_NIL;
	assert_type_hash_table(ecl_make_fixnum(/*EXT::HASH-TABLE-CONTENT*/1930), 2, ht);
	for (i = 0;  i < ht->hash.size;  i++) {
		struct ecl_hashtable_entry e = ht->hash.data[i];
		if (e.key != OBJNULL)
...
{
	assert_type_hash_table(ecl_make_fixnum(/*EXT::HASH-TABLE-FILL*/1931), 2, ht);
	while (!Null(values)) {
