#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/unixint.d"
...
#endif
#ifdef SIGINT
	{ SIGINT, "+SIGINT+", ECL_SYM("SI::TERMINAL-INTERRUPT",1161)},
#endif
#ifdef SIGQUIT
	{ SIGQUIT, "+SIGQUIT+", ECL_NIL},
#endif
#ifdef SIGILL
	{ SIGILL, "+SIGILL+", ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1673)},
#endif
#ifdef SIGTRAP
...
#endif
#ifdef SIGBUS
	{ SIGBUS, "+SIGBUS+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1676)},
#endif
#ifdef SIGSEGV
	{ SIGSEGV, "+SIGSEGV+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1676)},
#endif
#ifdef SIGSYS
...
#endif
#ifdef SIGCHLD
	{ SIGCHLD, "+SIGCHLD+", ECL_SYM("SI::WAIT-FOR-ALL-PROCESSES",1132)},
#endif
#ifdef SIGTTIN
...
interrupts_disabled_by_lisp(cl_env_ptr the_env)
{
	return !ecl_option_values[ECL_OPT_BOOTED] ||
		Null(ECL_SYM_VAL(the_env, ECL_SYM("EXT::*INTERRUPTS-ENABLED*",1023)));
}
...
handle_signal_now(cl_object signal_code, cl_object process)
{
        ...
        case t_fixnum:
                cl_cerror(4, str_ignore_signal, ECL_SYM("EXT::UNIX-SIGNAL-RECEIVED",1800),
                          ECL_SYM(":CODE",1802), signal_code);
                break;
        case t_symbol:
		/*
		...
		if (cl_find_class(2, signal_code, ECL_NIL) != ECL_NIL)
			...
			_ecl_funcall3(signal_code, ECL_SYM(":PROCESS",1294), process);
#endif
		else
...
handle_or_queue(cl_env_ptr the_env, cl_object signal_code, int code)
{
	 ...
	 */
		...
		si_trap_fpe(ECL_SYM("LAST",455), ECL_T); /* Clear FPE exception flag */
...
asynchronous_signal_servicing_thread()
{
	...
	for (;;) {
		...
		signal_code = ecl_gethash_safe(ecl_make_fixnum(signal_thread_msg.signo),
					       ...
					       ECL_NIL);
		if (!Null(signal_code)) {
			mp_process_run_function(4, ECL_SYM("SI::HANDLE-SIGNAL",1732),
						ECL_SYM("SI::HANDLE-SIGNAL",1732),
						signal_code,
...
handler_fn_prototype(fpe_signal_handler, int sig, siginfo_t *info, void *data)
{
	...
	unlikely_if (!ecl_option_values[ECL_OPT_BOOTED]) {
        ...
        unlikely_if (zombie_process(the_env))
                return;
	condition = ECL_SYM("ARITHMETIC-ERROR",93);
	code = 0;
#ifdef _MSC_VER
	switch (_fpecode) {
	case _FPE_INVALID:
		condition = ECL_SYM("FLOATING-POINT-INVALID-OPERATION",380);
		code = FE_INVALID;
		break;
	case _FPE_OVERFLOW:
		condition = ECL_SYM("FLOATING-POINT-OVERFLOW",381);
		code = FE_OVERFLOW;
		break;
	case _FPE_UNDERFLOW:
		condition = ECL_SYM("FLOATING-POINT-UNDERFLOW",382);
		code = FE_UNDERFLOW;
		break;
	case _FPE_ZERODIVIDE:
		condition = ECL_SYM("DIVISION-BY-ZERO",306);
		code = FE_DIVBYZERO;
		break;
	}
#else /* !_MSC_VER */
# if defined(HAVE_FENV_H) & !defined(ECL_AVOID_FENV_H)
	...
	if (code & FE_DIVBYZERO) {
		...
		condition = ECL_SYM("FLOATING-POINT-INEXACT",379);
		code = FE_INEXACT;
	}
...
# endif /* ECL_DOWN_STACK */
	...
	if (the_env->fault_address == info->si_addr) {
		...
		the_env->fault_address = info->si_addr;
		handle_or_queue(the_env, ECL_SYM("EXT::SEGMENTATION-VIOLATION",1676), sig);
	}
#else
	/*
...
{
        if (action == ECL_NIL || action == ECL_SYM(":IGNORE",1247)) {
                mysignal(code, SIG_IGN);
                return ECL_T;
        } else if (action == ECL_SYM(":DEFAULT",1215)) {
                mysignal(code, SIG_DFL);
                return ECL_T;
        } else if (action == ECL_SYM(":MASK",1271) || action == ECL_SYM(":UNMASK",1320)) {
#ifdef HAVE_SIGPROCMASK
# ifdef ECL_THREADS
...
# endif /* !ECL_THREADS */
	...
	} else if (action == ECL_T || action == ECL_SYM(":CATCH",1204)) {
                if (code == SIGSEGV) {
...
_ecl_w32_exception_filter(struct _EXCEPTION_POINTERS* ep)
{
	...
	switch (ep->ExceptionRecord->ExceptionCode)
	{
		...
		case EXCEPTION_INT_DIVIDE_BY_ZERO:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("DIVISION-BY-ZERO",306), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		case EXCEPTION_INT_OVERFLOW:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("ARITHMETIC-ERROR",93), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		case EXCEPTION_FLT_DIVIDE_BY_ZERO:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-OVERFLOW",381), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		...
		case EXCEPTION_FLT_UNDERFLOW:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-UNDERFLOW",382), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		case EXCEPTION_FLT_INEXACT_RESULT:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-INEXACT",379), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		case EXCEPTION_FLT_DENORMAL_OPERAND:
		case EXCEPTION_FLT_INVALID_OPERATION:
			feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-INVALID-OPERATION",380), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		...
		case EXCEPTION_ACCESS_VIOLATION:
                        handle_signal_now(ECL_SYM("EXT::SEGMENTATION-VIOLATION",1676), the_env->own_process);
                        return EXCEPTION_CONTINUE_EXECUTION;
		/* Catch illegal instruction */
		case EXCEPTION_ILLEGAL_INSTRUCTION:
			handle_signal_now(ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1673), the_env->own_process);
			return EXCEPTION_CONTINUE_EXECUTION;
...
{
	int outside_ecl = ecl_import_current_thread(ECL_SYM("SI::HANDLE-SIGNAL",1732), ECL_NIL);
	mp_process_run_function(4, ECL_SYM("SI::HANDLE-SIGNAL",1732),
				ECL_SYM("SI::HANDLE-SIGNAL",1732),
				signal_code, ECL_NIL);
...
BOOL WINAPI W32_console_ctrl_handler(DWORD type)
{
	switch (type)
	{
		/* Catch CTRL-C */
	case CTRL_C_EVENT: {
		cl_object function = ECL_SYM_FUN(ECL_SYM("SI::TERMINAL-INTERRUPT",1161));
		if (function)
	...
	for (;;) {
		...
		if (status == 0) {
			...
			signal_code = ecl_gethash_safe(ecl_make_fixnum(signo),
						       ...
						       ECL_NIL);
			if (!Null(signal_code)) {
				mp_process_run_function(3, ECL_SYM("SI::HANDLE-SIGNAL",1732),
							ECL_SYM("SI::HANDLE-SIGNAL",1732),
							signal_code);
			}
		}
...
# define FE_ALL_EXCEPT FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW | FE_INVALID
	...
	int bits = 0;
        if (condition == ECL_SYM("LAST",455)) {
		bits = the_env->trap_fpe_bits;
        } else {
                if (condition == ECL_T)
                        bits = FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW | FE_INVALID;
		else if (condition == ECL_SYM("DIVISION-BY-ZERO",306))
                        bits = FE_DIVBYZERO;
                else if (condition == ECL_SYM("FLOATING-POINT-OVERFLOW",381))
                        bits = FE_OVERFLOW;
                else if (condition == ECL_SYM("FLOATING-POINT-UNDERFLOW",382))
                        bits = FE_UNDERFLOW;
                else if (condition == ECL_SYM("FLOATING-POINT-INVALID-OPERATION",380))
                        bits = FE_INVALID;
                else if (condition == ECL_SYM("FLOATING-POINT-INEXACT",379))
                        bits = FE_INEXACT;
	...
	if (ecl_option_values[ECL_OPT_SIGNAL_HANDLING_THREAD]) {
		...
		cl_object fun =
			ecl_make_cfun((cl_objectfn_fixed)
				      asynchronous_signal_servicing_thread,
				      ECL_SYM("SI::SIGNAL-SERVICING",1765),
				      ECL_NIL,
				      0);
                nlogd("------------------------------ B");
		cl_object process = signal_thread_process = mp_process_run_function_wait(2, ECL_SYM("SI::SIGNAL-SERVICING",1765), fun); // this part call APPLY
                nlogd("------------------------------ C");
		if (Null(process)) {
...
# ifdef ECL_IEEE_FP
		...
		si_trap_fpe(ECL_SYM("FLOATING-POINT-INVALID-OPERATION",380), ECL_NIL);
		si_trap_fpe(ECL_SYM("DIVISION-BY-ZERO",306), ECL_NIL);
		si_trap_fpe(ECL_SYM("FLOATING-POINT-OVERFLOW",381), ECL_NIL);
# endif
	}
#endif
}
...
create_signal_code_constants()
{
	cl_object hash =
		cl_core.known_signals =
		cl__make_hash_table(ECL_SYM("EQL",334), ecl_make_fixnum(128),
				    cl_core.rehash_size,
...
init_unixint(int pass)
{
	if (pass == 0) {
		...
		install_signal_handling_thread(); ////////////////////////////// this part call apply
                nlogd(">>>>>>>4");
		ECL_SET(ECL_SYM("EXT::*INTERRUPTS-ENABLED*",1023), ECL_T);
                nlogd(">>>>>>>5");
