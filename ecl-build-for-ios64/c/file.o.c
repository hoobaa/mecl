...
clos_stream_read_byte8(cl_object strm, unsigned char *c, cl_index n)
{
    cl_index i;
    for (i = 0; i < n; i++) {
        cl_object byte = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1634), strm);
        if (!ECL_FIXNUMP(byte))
            break;
        c[i] = ecl_fixnum(byte);
    }
...
clos_stream_write_byte8(cl_object strm, unsigned char *c, cl_index n)
{
    cl_index i;
    for (i = 0; i < n; i++) {
        cl_object byte = _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1642), strm,
                                       ecl_make_fixnum(c[i]));
        if (!ECL_FIXNUMP(byte))
            break;
    }
...
{
    cl_object b = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1634), strm);
...
{
    _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1642), strm, c);
}

static ecl_character
clos_stream_read_char(cl_object strm)
{
    cl_object output = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-CHAR",1635), strm);
    cl_fixnum value;
...
{
    _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-CHAR",1643), strm, ECL_CODE_CHAR(c));
    return c;
}
...
{
    _ecl_funcall3(ECL_SYM("GRAY::STREAM-UNREAD-CHAR",1641), strm, ECL_CODE_CHAR(c));
}

static int
clos_stream_peek_char(cl_object strm)
{
    cl_object out = _ecl_funcall2(ECL_SYM("GRAY::STREAM-PEEK-CHAR",1633), strm);
...
{
    return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-LISTEN",1632), strm));
}

static void
clos_stream_clear_input(cl_object strm)
{
    _ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-INPUT",1622), strm);
}

static void
clos_stream_clear_output(cl_object strm)
{
    _ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-OUTPUT",1623), strm);
    return;
}
...
{
    _ecl_funcall2(ECL_SYM("GRAY::STREAM-FORCE-OUTPUT",1628), strm);
}

static void
clos_stream_finish_output(cl_object strm)
{
    _ecl_funcall2(ECL_SYM("GRAY::STREAM-FINISH-OUTPUT",1627), strm);
}

static int
clos_stream_input_p(cl_object strm)
{
    return !Null(_ecl_funcall2(ECL_SYM("GRAY::INPUT-STREAM-P",1618), strm));
}

static int
clos_stream_output_p(cl_object strm)
{
    return !Null(_ecl_funcall2(ECL_SYM("GRAY::OUTPUT-STREAM-P",1619), strm));
}

static int
clos_stream_interactive_p(cl_object strm)
{
    return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-INTERACTIVE-P",1630), strm));
...
{
    return _ecl_funcall2(ECL_SYM("GRAY::STREAM-ELEMENT-TYPE",1624), strm);
}
...
{
    return _ecl_funcall2(ECL_SYM("GRAY::STREAM-FILE-POSITION",1626), strm);
}

static cl_object
clos_stream_set_position(cl_object strm, cl_object pos)
{
    return _ecl_funcall3(ECL_SYM("GRAY::STREAM-FILE-POSITION",1626), strm, pos);
}

static int
clos_stream_column(cl_object strm)
{
    cl_object col = _ecl_funcall2(ECL_SYM("GRAY::STREAM-LINE-COLUMN",1631), strm);
...
{
    return _ecl_funcall2(ECL_SYM("GRAY::CLOSE",1616), strm);
}
...
#line 2323
  ...
  x = alloc_stream();
  if (Null(streams)) {
      x->stream.format = ECL_SYM(":PASS-THROUGH",1767);
  } else {
...
parse_external_format(cl_object stream, cl_object format, int flags)
{
    if (format == ECL_SYM(":DEFAULT",1215)) {
        format = ecl_symbol_value(ECL_SYM("EXT::*DEFAULT-EXTERNAL-FORMAT*",1766));
    }
    if (CONSP(format)) {
    ...
    if (format == ECL_SYM(":CR",1688)) {
        return (flags | ECL_STREAM_CR) & ~ECL_STREAM_LF;
    }
    if (format == ECL_SYM(":LF",1689)) {
        return (flags | ECL_STREAM_LF) & ~ECL_STREAM_CR;
    }
    if (format == ECL_SYM(":CRLF",1690)) {
        return flags | (ECL_STREAM_CR+ECL_STREAM_LF);
    }
    if (format == ECL_SYM(":LITTLE-ENDIAN",1699)) {
        return flags | ECL_STREAM_LITTLE_ENDIAN;
    }
    if (format == ECL_SYM(":BIG-ENDIAN",1700)) {
        return flags & ~ECL_STREAM_LITTLE_ENDIAN;
    }
    if (format == ECL_SYM(":PASS-THROUGH",1767)) {
#ifdef ECL_UNICODE
 ...
 PARSE_SYMBOLS:
    ...
    if (ECL_SYMBOLP(format)) {
        format = _ecl_funcall2(ECL_SYM("EXT::MAKE-ENCODING",1696), format);
        if (ECL_SYMBOLP(format))
            goto PARSE_SYMBOLS;
        stream->stream.format_table = format;
        return (flags & ~ECL_STREAM_FORMAT) | ECL_STREAM_USER_FORMAT;
                    ...
                    cl_object external_format)
{
    ...
    switch (flags & ECL_STREAM_FORMAT) {
        ...
        case ECL_STREAM_LATIN_1:
            ...
            byte_size = 8;
            stream->stream.format = ECL_SYM(":LATIN-1",1679);
        ...
        case ECL_STREAM_UTF_8:
            ...
            byte_size = 8;
            stream->stream.format = ECL_SYM(":UTF-8",1681);
        ...
        case ECL_STREAM_UCS_2:
            IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",222);
            byte_size = 8*2;
            stream->stream.format = ECL_SYM(":UCS-2",1682);
        ...
        case ECL_STREAM_UCS_2BE:
            ...
            if (flags & ECL_STREAM_LITTLE_ENDIAN) {
                stream->stream.format = ECL_SYM(":UCS-2LE",1693);
                stream->stream.encoder = ucs_2le_encoder;
                stream->stream.decoder = ucs_2le_decoder;
            } else {
                stream->stream.format = ECL_SYM(":UCS-2BE",1691);
                stream->stream.encoder = ucs_2be_encoder;
                stream->stream.decoder = ucs_2be_decoder;
            }
            break;
        case ECL_STREAM_UCS_4:
            IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",222);
            byte_size = 8*4;
            stream->stream.format = ECL_SYM(":UCS-4BE",1692);
        ...
        case ECL_STREAM_UCS_4BE:
            ...
            byte_size = 8*4;
            if (flags & ECL_STREAM_LITTLE_ENDIAN) {
                stream->stream.format = ECL_SYM(":UCS-4LE",1694);
                stream->stream.encoder = ucs_4le_encoder;
                stream->stream.decoder = ucs_4le_decoder;
            } else {
                stream->stream.format = ECL_SYM(":UCS-4BE",1692);
        ...
        case ECL_STREAM_US_ASCII:
            IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",120);
            byte_size = 8;
            stream->stream.format = ECL_SYM(":US-ASCII",1698);
        ...
        case ECL_STREAM_DEFAULT_FORMAT:
            IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",120);
            byte_size = 8;
            stream->stream.format = ECL_SYM(":PASS-THROUGH",1767);
            stream->stream.encoder = passthrough_encoder;
            stream->stream.decoder = passthrough_decoder;
            break;
#endif
...
si_set_buffering_mode(cl_object stream, cl_object buffer_mode_symbol)
{
    ...
    if (buffer_mode_symbol == ECL_SYM(":NONE",1598) || Null(buffer_mode_symbol))
        buffer_mode = _IONBF;
    else if (buffer_mode_symbol == ECL_SYM(":LINE",1601) || buffer_mode_symbol == ECL_SYM(":LINE-BUFFERED",1599))
        buffer_mode = _IOLBF;
    else if (buffer_mode_symbol == ECL_SYM(":FULL",1602) || buffer_mode_symbol == ECL_SYM(":FULLY-BUFFERED",1600))
        buffer_mode = _IOFBF;
    else
                        ...
                        cl_fixnum byte_size, int flags, cl_object external_format)
{
    ...
    nlogd(">>FUCK windows1. mode:%d", smm);
    switch(smm) {
        case ecl_smm_input:
            mode = OPEN_R;
            break;
...
# ifdef WORDS_BIGENDIAN
                external_format = ECL_SYM(":UCS-4BE",1692);
# else
                external_format = ECL_SYM(":UCS-4LE",1694);
# endif
            }
        }
#else
...
#ifdef ECL_CLOS_STREAMS
    if (ECL_INSTANCEP(strm)) {
        return _ecl_funcall2(ECL_SYM("GRAY::OPEN-STREAM-P",1620), strm);
    }
#endif
...
#ifdef ECL_CLOS_STREAMS
    if (ECL_INSTANCEP(strm)) {
        return _ecl_funcall2(ECL_SYM("GRAY::STREAMP",1617), strm);
    }
#endif
    {
#line 4999
                ...
                int flags, cl_object external_format)
{
    ...
    if (flags & ECL_STREAM_C_STREAM) {
        ...
        output = ecl_make_stream_from_FILE(fn, fp, smm, byte_size, flags,
                                           external_format);
        si_set_buffering_mode(output, byte_size? ECL_SYM(":FULL",1602) : ECL_SYM(":LINE",1601));
    } else {
...
{
    cl_object code = _ecl_funcall4(ECL_SYM("EXT::ENCODING-ERROR",1829), stream,
...
decoding_error(cl_object stream, unsigned char *buffer, int length)
{
    ...
    code = _ecl_funcall4(ECL_SYM("EXT::DECODING-ERROR",1828), stream,
                         cl_stream_external_format(stream),
                         octets);
    if (Null(code)) {
...
init_file(void)
{
    ...
    error_output = ecl_make_stream_from__FILE(make_constant_base_string("stderr"),
                                              stderr, ecl_smm_output, 8, flags, external_format);
#endif
    cl_core.standard_input = standard_input;
    ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-INPUT+",1809), standard_input);
    ECL_SET(ECL_SYM("*STANDARD-INPUT*",66), standard_input);
    cl_core.standard_output = standard_output;
    ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-OUTPUT+",1810), standard_output);
    ...
    ECL_SET(ECL_SYM("EXT::+PROCESS-ERROR-OUTPUT+",1811), error_output);
