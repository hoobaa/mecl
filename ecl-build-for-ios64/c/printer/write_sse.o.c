...
write_sse_pack(cl_object x, cl_object stream)
{
	...
	cl_object mode = ecl_symbol_value(ECL_SYM("EXT::*SSE-PACK-PRINT-MODE*",1799));

	if (mode != ECL_NIL) {
		if (mode == ECL_SYM(":FLOAT",1368)) etype = ecl_aet_sf;
		else if (mode == ECL_SYM(":DOUBLE",1366)) etype = ecl_aet_df;
