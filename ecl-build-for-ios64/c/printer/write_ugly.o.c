#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/printer/write_ugly.d"
...
{
        cl_object l =
                cl_list(15, ECL_SYM("MAKE-PATHNAME",531),
                        ECL_SYM(":HOST",1243), path->pathname.host,
                        ECL_SYM(":DEVICE",1217), path->pathname.device,
                        ECL_SYM(":DIRECTORY",1219),
                        _ecl_funcall2(ECL_SYM("EXT::MAYBE-QUOTE",1686), path->pathname.directory),
                        ECL_SYM(":NAME",1273), path->pathname.name,
                        ECL_SYM(":TYPE",1318), path->pathname.type,
                        ECL_SYM(":VERSION",1326), path->pathname.version,
                        ECL_SYM(":DEFAULTS",1216), ECL_NIL);
...
write_integer(cl_object number, cl_object stream)
{
        ...
        si_integer_to_string(s, number,
                             ...
                             ecl_symbol_value(ECL_SYM("*PRINT-RADIX*",56)),
...
{
	if (ecl_print_readably() && !Null(ecl_symbol_value(ECL_SYM("*READ-EVAL*",63)))) {
		cl_object make =
			cl_list(9, ECL_SYM("MAKE-HASH-TABLE",528),
				ECL_SYM(":SIZE",1308), cl_hash_table_size(x),
				ECL_SYM(":REHASH-SIZE",1297), cl_hash_table_rehash_size(x),
				ECL_SYM(":REHASH-THRESHOLD",1298), cl_hash_table_rehash_threshold(x),
				ECL_SYM(":TEST",1316), cl_hash_table_test(x));
		cl_object init =
			cl_list(3, ECL_SYM("EXT::HASH-TABLE-FILL",1931), make,
				cl_list(2, ECL_SYM("QUOTE",679), si_hash_table_content(x)));
		writestr_stream("#.", stream);
...
{
        _ecl_funcall3(ECL_SYM("PRINT-OBJECT",961), x, stream);
}
#else
static void
write_structure(cl_object x, cl_object stream)
{
        ...
        unlikely_if (ecl_t_of(x->str.name) != t_symbol)
                FEerror("Found a corrupt structure with an invalid type name~%"
                        "  ~S", x->str.name);
        print_function = si_get_sysprop(x->str.name, ECL_SYM("SI::STRUCTURE-PRINT-FUNCTION",1152));
