#line 1 "/Users/strobolights/dev/mecl/ecl/src/c/num_rand.d"
...
rando(cl_object x, cl_object rs)
{
	...
	default: ERROR: {
		...
		FEwrong_type_nth_arg(ecl_make_fixnum(/*RANDOM*/680),1,x, ecl_read_from_cstring(type));
        }
	}
	return z;
}
...
ecl_make_random_state(cl_object rs)
{
	...
	if (rs == ECL_T) {
		...
		if (Null(rs)) {
			rs = ecl_symbol_value(ECL_SYM("*RANDOM-STATE*",60));
		}
		unlikely_if (!ECL_RANDOM_STATE_P(rs)) {
			FEwrong_type_only_arg(ecl_make_fixnum(/*MAKE-RANDOM-STATE*/532), rs,
                                              ecl_make_fixnum(/*RANDOM-STATE*/681));
		}
